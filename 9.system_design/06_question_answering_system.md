...
## Problem Statement
Design a QA system that answers user questions over a large document corpus.
Scale: 10M documents, 50K QPS, multi-turn conversations, streaming responses.
...
**Retrieval → RAG reference:** `9.system_design/03_search_and_rag_system.md` for FAISS, chunking, and embedding pipeline details.
...
Standalone question (one sentence, no pronouns referring to prior context):"""
        }]
    )
...
history = [
    ...
    {"role": "assistant", "content": "Standard payment term is Net 30 days."},
]
...
```
LLM context limit: 128K tokens (Claude), 32K (GPT-4), 8K (older models)
...
Offline metrics:
  ...
  Answer quality:
    ...
    ROUGE-L:      overlap with reference answers (if available)
...
def check_faithfulness(answer: str, context: str) -> float:
    ...
    # Returns: entailment (faithful), neutral, contradiction (hallucination)
