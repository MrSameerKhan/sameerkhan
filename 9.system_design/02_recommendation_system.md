...
## Problem Statement
Design a recommendation system for [Netflix / Amazon / LinkedIn / YouTube].
Scale: 100M users, 10M items, 100K QPS.
...
│                    ONLINE (real-time)                            │
│                                                                  │
│  Request → [RETRIEVAL: get top-k candidates]                    │
...
class UserTower(nn.Module):
    def __init__(self, user_vocab_size, item_vocab_size, embed_dim=128):
        ...
        self.item_history_embed = nn.EmbeddingBag(item_vocab_size, 64, mode='mean')
...
def two_tower_loss(user_emb, item_emb_pos, temperature=0.05):
    ...
    All other items in batch = negatives
    """
...
# Retrieval at serving time (sub-millisecond for 10M items)
...
def get_recommendations(user_id: int):
    ...
    if len(user_history) == 0:
        ...
        popular_recs = get_popular_items(k=300)
...
def get_item_embedding(item_id: int):
    if item_is_new(item_id):
        # Use content-based embedding (text, image) until behavioral data accumulates
...
Watch out for:
  Feedback loop: recommending popular items → more data on popular items → only popular items get recommended
...
A: The feedback loop: recommended popular items → more clicks on popular items → model trained on these clicks → popular items recommended more. Over time, catalog diversity collapses. Solutions: (1) Exploration: ε-greedy (5% of recommendations are random) or Upper Confidence Bound (explore uncertain items), (2) Inverse propensity weighting — down-weight popular item interactions in training, (3) Freshness signals — add recency decay to item scores, boost new items, (4) Diversity constraints in reranking — max k items per category, (5) Counterfactual learning — train on what would have been clicked had different items been shown. No single solution eliminates it; use in combination.
...
- **RAG (5.llms/04):** ANN retrieval (FAISS, ScaNN) same techniques used in both
- **System Design Framework (8.system_design/01):** Apply the framework here
