...
Scale:
  ...
  - Expected query volume: 100 QPS? 10K QPS?
...
The critical point: offline metric ≈ online metric, but not exactly.
Always justify your offline metric choice in terms of the business goal.
...
If inference alone takes 80ms → need optimization:
  - Quantization: 2-4× speedup
  - Batching: amortize fixed costs
  - Caching: cache features, cache results for frequent queries
...
Caching:
  ...
  Result cache: cache recommendations for repeat queries
...
Cold Start:
  "New users with no history → fall back to popularity-based recommendation.
   ...
   After 3+ interactions, blend collaborative filtering in."

Scalability:
  "At 1K QPS this design works with 2 replicas.
