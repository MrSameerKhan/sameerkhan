# Search & RAG System Design

## Problem Statement
Design a semantic search system for a document corpus (e.g., enterprise knowledge base, legal docs, internal wikis).
Scale: 10M documents, 10K QPS, latency < 200ms.
...
│                                                                  │
│  Query → [Query Processing] → [Hybrid Retrieval]               │
│       → [Reranking] → [Generation (for RAG)] → Response        │
└──────────────────────────────────────────────────────────────────┘
```
...
class HybridRetriever:
    ...
    def retrieve(self, query: str, filters: dict = None) -> list:
        ...
        sparse_top_k = np.argsort(bm25_scores)[-self.k:][::-1]
...
A: Query decomposition + multi-document synthesis: (1) Use LLM to decompose complex query into sub-questions; (2) Retrieve and rerank separately for each sub-question; (3) Deduplicate across sub-queries; (4) In generation prompt, explicitly instruct LLM to synthesize information from multiple sources; (5) Return citations for each source document. For conflicting information across documents (common in legal/policy documents), instruct the LLM to surface the conflict rather than arbitrarily choosing one.

**Q: How do you prevent the RAG system from hallucinating?**
A: Layered defense: (1) Retrieval quality — if relevant context isn't retrieved, generation will hallucinate; improve retrieval with hybrid search + reranking; (2) Prompt — "Answer ONLY using the provided context. If not in context, say 'I don't have information'"; (3) Faithfulness scoring — use NLI model or LLM judge to verify each claim in the answer is supported by retrieved context; (4) Citation enforcement — require the model to cite specific chunks; claims without citations are flagged; (5) Confidence thresholding — if top retrieved chunk similarity is below threshold, return "I don't have enough information" rather than generating from weak context.
