...
Computing each output position (dot product of 3×3 patch with W):
...
```
p = σ(1.36) = 1 / (1 + e^{-1.36}) = 1 / (1 + 0.257) = 0.795
...
```
dL/d(logit) = p - y = 0.795 - 1.0 = -0.205
```
...
dL/dW2 = a^T × dL/d(logit)
       = [[2.1], [0.0]] × [-0.205]
       = [[-0.431], [0.0]]
  W2[0] gets gradient, W2[1] gets 0 (dead neuron)
...
Neurons corresponding to flatten[1]=0 and flatten[3]=0
→ zero gradient (zero input = dead for weight update)
```
...
dL/dW_conv[i,j] = Σ dL/dZ[r,c] × X[r+i, c+j]

Using only the non-zero gradients (dL/dZ[0,0]=-0.0615 and dL/dZ[1,0]=-0.0492):

dL/dW_conv[0,0] = -0.0615×1 + (-0.0492)×2 = -0.0615 - 0.0984 = -0.1599
dL/dW_conv[0,1] = -0.0615×0 + (-0.0492)×1 = 0 - 0.0492 = -0.0492
dL/dW_conv[0,2] = -0.0615×2 + (-0.0492)×0 = -0.1230
dL/dW_conv[1,0] = -0.0615×2 + (-0.0492)×0 = -0.1230
...
W2     ← W2     - 0.01 × dL/dW2
  ...
  W2[1] = -0.4 - 0.01×0 = -0.4                             (unchanged — dead neuron)
...
Examples:
  Conv 3×3, C_in=3,   C_out=64:   3×3×3×64    = 1,792 params
  Conv 3×3, C_in=64,  C_out=128:  3×3×64×128  = 73,856 params
  Conv 1×1, C_in=256, C_out=64:   1×1×256×64  = 16,384 params (bottleneck)
```
...
Input:  [[1, 2, 1, 3],     Window [0:2, 0:2]: max=4 (position (1,0))
          [4, 2, 1, 3],     Window [0:2, 2:4]: max=3 (position (1,3))
          [1, 0, 3, 1],     Window [2:4, 0:2]: max=3 (position (3,1))
          [2, 3, 0, 2]]     Window [2:4, 2:4]: max=3 (position (2,2))
...
train_transforms = T.Compose([
    ...
    T.RandomHorizontalFlip(),                      # mirror image
...
| Object detection | mAP@50:95 | mAP averaged over IoU 0.5,0.55,...,0.95 |
...
**He initialization for ReLU layers.** Using Xavier (for sigmoid/tanh) with ReLU → kills activations → dead neurons.

**Batch normalization placement.** Standard: Conv → BN → ReLU. Pre-activation ResNets use BN → ReLU → Conv → slightly better gradient flow. Don't apply BN after final FC layer in generation tasks.
...
**Q: What does a backward pass through MaxPool look like?**
...
| LayoutLM (CNN + transformer for documents) | `../../9.multimodal/06_layout.md` |
...
```
Forward: Conv (cross-correlation) → ReLU (nonlinearity, kills negatives)
         → Pool (downsample, invariance) → FC (decision)

Backward: gradients flow through ReLU gates (zero where Z<0)
          Pool routes gradient to max position only
          Conv filter gradient = correlate input with upstream gradient

Dead neurons (always-zero ReLU) give zero gradient → don't update
→ use He initialization and batch normalization to prevent
```
