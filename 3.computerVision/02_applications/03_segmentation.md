...
- Instance: mAP at IoU thresholds (same as detection mAP for masks)
...
Problem: 14×14 → 224×224 bilinear upsampling loses fine detail.
...
**Designed for:** biomedical image segmentation with limited labeled data. **Authors:** Ronneberger et al., 2015.
...
Encoder (Contracting Path):            Decoder (Expanding Path):
  ...
  256×256×64                             256×256×128 (cat + conv×2)
        ↓               skip             Upsample
  [Conv3×3+ReLU]×2 + MaxPool  +++++++++  128×128×64  (cat + conv×2)
  128×128×128                            Upsample
        ↓               skip             64×64×32  (cat + conv×2)
  [Conv3×3+ReLU]×2 + MaxPool  +++++++++  Upsample
  64×64×256                              32×32×16  (cat + conv×2)
        ↓               skip             Output
  [Conv3×3+ReLU]×2 + MaxPool  +++++++++  1×1 conv → num_classes
  32×32×512                              + sigmoid/softmax → mask
        ↓
  Bottleneck: [Conv]×2
  16×16×1024
```
...
class UNet(nn.Module):
    def __init__(self, in_channels=3, num_classes=1):
        ...
        # Decoder
        self.up4  = nn.ConvTranspose2d(1024, 512, 2, stride=2)  # 1024 → 512 from upsample + 512 from skip
...
```
Option 1: Remove stride → feature map stays large but RF shrinks (can't see large context)
...
For each ROI (aligned to 14×14):
  4× Conv2×3 → ConvTranspose2d (upsample to 28×28) → Conv1×1 → sigmoid
...
MLP Decoder:
  ...
  Concatenate → MLP → upsample 4× → output
...
**SAM2 (2024) — Video Segmentation:** Extends SAM to video; given a prompt in one frame, tracks the mask throughout the video. Memory attention mechanism stores object memory from previous frames.

**SAM Limitations:** Cannot label what the segment IS (no classification). Struggles with very thin structures (wires, hair). Slow for real-time (ViT-H encoder is large). SAM2 adds video support but same classification limitation.

---

## 7.5 Mask2Former and OneFormer — Universal Segmentation

Pre-2022: needed different architectures for semantic vs instance vs panoptic. **Mask2Former** (Meta, 2022) unified all three — treating each as "predict N (class, mask) pairs".

```
Universal segmentation formulation:
  N learned object queries → transformer decoder → for each query:
    class probability (or "no object")
    binary mask (sigmoid)

  Semantic:  group masks by class
  Instance:  one (class, mask) per object
  Panoptic:  both
```

| Model | Year | Notes |
|-------|------|-------|
| MaskFormer | 2021 | First "predict mask classes" formulation |
| Mask2Former | 2022 | + masked attention, multi-scale deformable attention; SOTA for years |
| OneFormer | 2023 | Single model trained jointly on all three tasks |
| OpenSeeD | 2023 | Open-vocabulary version |
...
panoptic_seg = processor.post_process_panoptic_segmentation(outputs, target_sizes=[image.shape[-2:]])[0]
```

---

## 7.6 Grounded-SAM and Open-Vocabulary Segmentation (2023-2024)

SAM segments but doesn't classify. **Grounded-SAM** combines Grounding DINO (text-prompted detection) with SAM (promptable segmentation) for text-conditioned segmentation.

```
Pipeline:
  text prompt "a red car, a stop sign"
        ↓
  Grounding DINO → bounding boxes for each phrase
        ↓
  SAM (use boxes as prompts) → precise masks
        ↓
  output: (text label, segmentation mask) per detected object
```

Other modern open-vocabulary segmentation: **SEEM** (Segment Everything Everywhere with Multimodal prompts) — points, boxes, text, scribbles, all in one model. **FC-CLIP** — close-vocab + open-vocab unified. **SAM 2** (Meta, 2024) — extends SAM to video with memory module; one prompt segments object across all frames.

For document AI: Grounded-SAM enables "segment all field labels and their corresponding values" without any annotated training data.
...
**mIoU is heavily affected by rare classes.** A model that ignores all small-area rare classes (medical anomalies, traffic signs) can still achieve mIoU=0.85+. Always report per-class IoU alongside mIoU. Weight rare classes or use Dice loss.

**Upsampling artifacts at boundaries.** Bilinear upsampling produces checkerboard artifacts for sharp boundaries. Use transposed convolution (ConvTranspose2d) or sub-pixel convolution (PixelShuffle) for cleaner boundaries.

**U-Net skip connection channel mismatch.** Encoder output channels must match decoder input before concatenation. Debug by printing shapes at each concat. Common error: mismatched after modifying encoder depth or channel sizes.

**Semantic segmentation ignores instance boundaries.** If two cars touch, semantic segmentation merges them. If your use case requires distinguishing instances (parking lot car counting), use Mask R-CNN or panoptic segmentation.

**SAM doesn't know what the segment IS.** SAM outputs a mask, not a class. For document zone detection, you need to add a classification step after SAM segments regions, or use it in tandem with a classifier.
...
| DETR deep dive (Mask2Former lineage) | `06_detr_deep.md` | Query-based detection / segmentation |
| Dice loss derivation | `../../2.deep learning/01_fundamentals/06_specialized_losses.md` | Dice loss for imbalanced segmentation |
| ViT as encoder (SegFormer, SAM) | `../../2.deep learning/02_architectures/04_transformer.md` | Transformer encoder in vision |
| ViT/Swin deep dive | `../01_fundamentals/04_vision_transformer_deep.md` | Window attention, hierarchical patches |
| Skip connections in U-Net | `../../2.deep learning/01_fundamentals/05_modern_components.md` | Residual/skip connection concept |
| Self-supervised vision pretraining | `05_self_supervised_vision.md` | DINOv2 features for segmentation |
...
Loss function choice:
  BCE for balanced classes
  Dice for imbalanced (most segmentation tasks)
  BCE+Dice combined for best results
```
