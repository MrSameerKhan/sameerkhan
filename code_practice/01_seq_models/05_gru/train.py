...
Compare val PPL vs Session 3 LSTM.
"""
...
def compute_val_ppl(model, val_data, seq_len, n_seqs=VAL_SEQS):
    """Forward-only on n_seqs random val windows. Returns PPL."""
    total_loss = 0.0
    ...
    for _ in range(n_seqs):
        ...
        # Use forward only — extract loss from ps without running backward
        _, _, _, _, _, _, hs, ps = model.forward(inputs, h)
...
def main():
    ...
    model = VanillaGRU(vocab_size, HIDDEN_SIZE, lr=LR, seed=SEED)
    total_params = (3 * HIDDEN_SIZE * (HIDDEN_SIZE + vocab_size) +
                    3 * HIDDEN_SIZE +
                    vocab_size * HIDDEN_SIZE + vocab_size)
    print(f"Parameters    : {total_params:,}  (LSTM has ~{4*HIDDEN_SIZE*(HIDDEN_SIZE+vocab_size)//1000}K)\n")
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        loss, h, grads = model.backward(inputs, targets, h)
        model.update(grads)
