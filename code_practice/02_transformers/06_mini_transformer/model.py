...
model.py: Stack N encoder blocks → MiniTransformerEncoder.

Architecture:
    Embedding → [EncoderBlock × N] → final LayerNorm → masked mean pool → Linear
"""
...
# ─── Mini Transformer Encoder (N stacked blocks) ─────────────────────────────

class MiniTransformerEncoder(nn.Module):
    """
    Embedding → [TransformerEncoderBlock × num_layers] → LayerNorm → masked mean pool → Linear.
    Returns (logits, all_weights) where all_weights is a list of length num_layers,
    each element [B, H, T, T].
    """
    def __init__(self, vocab_size: int, d_model=64, num_heads=8,
                 d_ff=256, n_classes=5, num_layers=4, max_len=512, dropout=0.0):
        super().__init__()
    ...
    def forward(self, token_ids, pad_mask=None):
        ...
        for block in self.blocks:
            h, weights = block(h, pad_mask)
            all_weights.append(weights)    # each: [B, H, T, T]

        h = self.ln_out(h)

        if pad_mask is not None:
            real   = (~pad_mask).unsqueeze(-1).float()
            pooled = (h * real).sum(1) / real.sum(1)
        else:
            pooled = h.mean(dim=1)

        return self.fc(pooled), all_weights   # [B, n_classes], list of [B, H, T, T]
