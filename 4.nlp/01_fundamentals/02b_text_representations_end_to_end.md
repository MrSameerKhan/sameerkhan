...
| To BERT (5.transformers/models/05_bert_end_to_end.md) | — | BERT produces contextual dense representations — TF-IDF's semantic limitation is fully solved — But BERT's output for classification is the same final step as TF-IDF — The pipeline is: TF-IDF → LogReg (fast, interpretable) vs BERT → LogReg (slow, accurate) |
