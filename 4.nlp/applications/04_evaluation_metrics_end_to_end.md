...
| True≠neg | FP=1 (row 1 — predicted neg, was pos) | TN=5 |
...
**Rule of thumb:** rare disease detection → Macro (every class matters equally).  
Search ranking → Micro (volume of correct predictions matters).
...
Candidate tokens: {cat, mat, on, sat}
...
"cat on mat" is a subsequence of "cat sat on mat" (skip "sat").
...
| B: "cat mat on sat"     | **1.000** | **0.000** | **0.500** |
...
Perplexity is vocabulary-size dependent. A model with vocab=50k has higher baseline PP than one with vocab=1k. Only compare PP across models with the same vocab and tokenizer.
...
ROUGE-1: "rested" ≠ "sat" → loses one match
...
Step 3: For each candidate token: find max cosine similarity to any reference token
Step 4: For each reference token: find max cosine similarity to any candidate token
Step 5: Average to get precision/recall/F1
```

**Example (conceptual, not real BERT numbers):**
...
def classification_report_manual(y_true, y_pred):
    ...
    for cls in classes:
        ...
        results[cls] = {'TP': TP, 'FP': FP, 'FN': FN, 'P': round(P,3), 'R': round(R,3), 'F1': round(F1,3)}
...
for cls, r in results.items():
    print(f"{cls}: P={r['P']} R={r['R']} F1={r['F1']}")
...
def modified_precision(candidate, references, n):
    ...
    if sum(cand_ngrams.values()) == 0:
        return 0
    # max count for each ngram across all references
    max_ref_counts = {}
    for ref in references:
        ref_ngrams = Counter(ngrams(ref, n))
        for ng in cand_ngrams:
            max_ref_counts[ng] = max(max_ref_counts.get(ng, 0), ref_ngrams.get(ng, 0))
    clipped = sum(min(count, max_ref_counts.get(ng, 0)) for ng, count in cand_ngrams.items())
...
def bleu(candidate_str, reference_strs, max_n=4):
    ...
    for i, p in enumerate(precisions, 1):
        print(f"  p_{i} = {p:.3f}")
    # log average (skip zeros for display but they kill score)
...
print(f"  BLEU-2 = {score_a}\n")

print("Candidate B:")
score_b = bleu("cat sat", reference, max_n=2)
print(f"  BLEU-2 = {score_b}")
...
print(f"\nAverage NLL = {-np.mean(log_probs):.3f}")
print(f"Perplexity  = {pp}")
print(f"\nReference: random (vocab=4) → PP = {round(np.exp(np.log(4)), 2)}")
...
Candidate: 'cat mat on sat'
  ROUGE-1: {'precision': 1.0, 'recall': 1.0, 'f1': 1.0}  ← misleading!
...
> Precision: of everything we labeled positive, how many actually were? (false alarm rate)  
> Recall: of all true positives, how many did we find? (miss rate)  
> High recall, low precision = spam filter that blocks too much.  
> High precision, low recall = spam filter that lets spam through.

**Q: When would you prefer recall over precision?**
> When false negatives are more costly than false positives:  
> Cancer screening — missing a cancer case (FN) is worse than a false alarm (FP) → maximize recall.  
> Spam filtering — blocking a real email (FP) might be worse than missing spam (FN) → maximize precision.

**Q: What's macro vs micro F1?**
> Macro: average per-class F1. Each class counts equally. Use when you care about rare classes.  
> Micro: global TP/FP/FN pooled. Dominated by frequent classes. Use when volume matters.

**Q: Why is BLEU often 0 for short sentences?**
> BLEU-4 requires 4-gram matches. A 4-word sentence only has one possible 4-gram.  
> If that 4-gram doesn't match exactly, BLEU-4 = 0. Use BLEU-2 for short sentences or corpus-level BLEU.

**Q: What does ROUGE-1 = 1.0 but ROUGE-L = 0.5 tell you?**
> Same words, wrong order. ROUGE-1 only counts token overlap, not sequence.  
> ROUGE-L uses LCS — captures ordering. Always report ROUGE-L alongside ROUGE-1.

**Q: Why does BLEU use brevity penalty?**
> Without it, a 1-word candidate "cat" would have 100% precision: p_1=1/1=1, BLEU=1.0.  
> Brevity penalty: BP = exp(1 - r/c) when candidate is shorter than reference.  
> For "cat" vs "cat sat on mat": BP = exp(1-4/1) = exp(-3) = 0.050. BLEU → near 0. ✓

**Q: What's the relationship between perplexity and cross-entropy loss?**
> PP = exp(H) where H = average cross-entropy loss.  
> The "loss" your trainer prints IS H. PP = exp(loss).  
> If loss = 2.3 → PP ≈ 10. If loss = 0.5 → PP ≈ 1.6.

**Q: When would you use BERTScore over ROUGE?**
> When paraphrasing is acceptable: "automobile" should match "car", "rested" should match "sat".  
> ROUGE only does exact token matching. BERTScore matches via cosine similarity in embedding space.  
> Tradeoff: BERTScore is slower, less interpretable, and requires a BERT model.
