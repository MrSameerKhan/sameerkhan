...
# Add entity markers to tokenizer vocabulary
special_tokens = ['[E1]', '[/E1]', '[E2]', '[/E2]']
tokenizer.add_special_tokens({'additional_special_tokens': special_tokens})
model.resize_token_embeddings(len(tokenizer))

# Classification: use [CLS] representation + entity representations
class REModel(torch.nn.Module):
    def __init__(self, encoder, num_relations, hidden_size=768):
        super().__init__()
        self.encoder = encoder
        # Use [CLS] + both entity representations (3 * hidden_size)
...
This is the most practically relevant section for document automation work.
...
boxes = [[10,20,80,35], [85,20,95,35], [100,20,150,35],
         [10,50,65,65], [70,50,110,65], [115,50,150,65]]
...
A: NER finds entity mentions in text and labels their type (open-domain: PER, ORG, LOC). Slot filling maps text to a predefined schema (closed-domain: INVOICE_NUMBER=INV-001, AMOUNT=$500). In practice, document extraction blurs this line: you define custom entity types (INVOICE_NUM, VENDOR) that match your schema, so it's both. QA-based slot filling treats each slot as a reading comprehension question, which handles flexible document structure better than fixed IOB tagging.

**Q: How do you handle documents where layout matters (e.g., the word "Total" means different things in different positions)?**
A: This is exactly the use case for LayoutLM. Standard BERT sees "Total: $500" as a flat string regardless of where it appears. LayoutLMv3 adds 2D position embeddings from bounding box coordinates, so "Total" in the bottom-right corner of an invoice is distinguished from "Total" in a header. For multi-column documents, this positional context is essential.

**Q: How do you evaluate an information extraction system in production?**
A: Multiple levels: (1) Component-level — NER F1 per entity type, RE accuracy per relation type. (2) End-to-end field accuracy — exact match and normalized match per document field. (3) Business-level — % of documents processed without human review (straight-through processing rate), exception rate, time saved. (4) Error analysis — categorize failures: OCR errors, NER boundary errors, RE errors, normalization failures. Each has different fixes. Monitor field-level accuracy in production with sampled human review.
...
## Connections
- **NER (applications/02):** NER is the foundation of most IE pipelines — entities are extracted first
- **Text Classification (applications/01):** Document type classification often precedes field extraction
- **Transformers (transformers/01):** LayoutLMv3 extends BERT with 2D position embeddings
- **Evaluation Metrics (applications/04):** Field-level F1, exact match, normalized match
...
## Key Takeaway
IE is where NLP meets the real world. For document automation: LayoutLMv3 when you have labeled training data and layout matters; Donut or LLM prompting when you don't. The hardest engineering is downstream of the model: OCR error handling, entity normalization (dates, amounts, names), confidence thresholding, and human-in-the-loop for exceptions. Build the pipeline end-to-end before optimizing any one component.
