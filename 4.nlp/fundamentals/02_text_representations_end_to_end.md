...
D2: [0.203, 0.000, 0.000, 0.101]   ← cat-heavy, no mat
D3: [0.000, 0.304, 0.000, 0.000]   ← pure mat signal
```

**Reading the matrix:**
- D1 is characterized equally by cat, mat, sat
- D2 is most strongly characterized by cat (TF-IDF=0.203)
- D3 is most strongly characterized by mat (TF-IDF=0.304)
- "on" contributes nothing to any document (all zeros in that column)
...
**Interpretation:**
- D1 ("cat sat on mat") is most similar to D2 ("cat sat on cat") — they share cat AND sat
...
Notice: all three share "on" but it's zeroed out by IDF. The similarity is driven entirely by the informative terms cat, mat, sat.
...
Feature vector for D1 (unigrams + bigrams):
  ...
  Bigrams:  [cat sat=1, mat mat=0, mat on=0, on cat=0, on mat=1, sat on=1]
  Combined: [1, 1, 1, 1, 1, 0, 0, 0, 1, 1]   (10-dimensional)
```
...
From our corpus (D1 only):
  P(sat | cat)  = count("cat sat") / count("cat") = 1/1 = 1.000
  P(on  | sat)  = count("sat on")  / count("sat") = 1/1 = 1.000
  P(mat | on)   = count("on mat")  / count("on")  = 1/1 = 1.000
...
Compare to standard IDF: IDF(on)=0.000, IDF_bm25(on)=0.134 — BM25 doesn't fully zero out common terms.
...
denominator: 1 + 1.5 × (1 - 0.75 + 0.75 × 4/4) = 1 + 1.5 × (0.25 + 0.75) = 1 + 1.5 = 2.500
...
```
denominator becomes: TF + k1 × 1.0 = TF + 1.5   (no |d|/avgdl term)
```
...
Cosine similarity with any document = 0. The sentence is invisible.
...
**Where this hurts:**
- Paraphrase detection: "She's happy" vs "She's joyful" → cosine=0
- Cross-lingual retrieval: English query, Spanish documents → cosine=0
- Domain shift: train on news, test on social media (different vocabulary)

### 8.2 Where Sparse Representations Win

Despite their limitations, sparse representations dominate in:

**Keyword search (Elasticsearch, Lucene, Solr):**
The query "machine learning" should literally appear in relevant documents. BM25 with inverted index scales to billions of documents at millisecond latency.

**Exact term matching:**
Legal documents: "Section 12.3.b" must match literally — semantics don't help here.

**Low-data settings:**
100 labeled documents → TF-IDF + Logistic Regression is often competitive with BERT because BERT fine-tuning overfits on small data.

**Interpretability:**
You can look at the top TF-IDF features for a prediction and explain why the model made a decision. BERT doesn't offer this.
...
z = D1_tfidf · w + b
  ...
  = 0.081  + (-0.030)          + 0.000          + 0.051
  = 0.102

P(cat class) = σ(0.102) = 1/(1+e^{-0.102}) = 1/(1+0.903) = 0.525
```
...
idf = np.log(N / df)                           # standard IDF
idf[np.isinf(idf)] = 0.0                       # handle zero df (shouldn't happen)
print("\nIDF vector:", dict(zip(vocab, idf.round(3))))

# Step 6: TF-IDF
tfidf = tf * idf
print("\nTF-IDF matrix:")
for i, doc in enumerate(docs):
    print(f"  D{i+1}: {dict(zip(vocab, tfidf[i].round(3)))}")
...
print(f"  D1 vs D2: {cosine_sim(tfidf[0], tfidf[1]):.3f}")
print(f"  D1 vs D3: {cosine_sim(tfidf[0], tfidf[2]):.3f}")
print(f"  D2 vs D3: {cosine_sim(tfidf[1], tfidf[2]):.3f}")
```
...
print(f"Unigram + bigram vocabulary ({len(ngram_vocab)} features):")
print(ngram_vocab)

# Build n-gram count matrix
ngram_bow = np.zeros((len(docs), len(ngram_vocab)))
for i, tokens in enumerate(tokenized):
    for n in range(1, 3):
        for gram in get_ngrams(tokens, n):
            ngram_bow[i][ngram2idx[gram]] += 1

print("\nN-gram BoW matrix (first 6 features shown):")
for i, doc in enumerate(docs):
...
for doc_idx, score in ranked:
    print(f"  D{doc_idx+1}: {docs[doc_idx]} — score={score:.3f}")
```
...
def bm25_scores(docs, query, k1=1.5, b=0.75):
    ...
    Returns array of shape (N,) with one score per document.
    """
...
for i, (doc, score) in enumerate(zip(docs, bm25)):
    print(f"  D{i+1}: {doc} → {score:.3f}")

# Compare with TF-IDF
from sklearn.feature_extraction.text import TfidfVectorizer
from sklearn.metrics.pairwise import cosine_similarity

tfidf_vec = TfidfVectorizer(smooth_idf=False, norm=None)
X = tfidf_vec.fit_transform(docs)
query_vec = tfidf_vec.transform([query])
tfidf_scores = cosine_similarity(query_vec, X)[0]

print(f"\nComparison for query '{query}':")
print(f"{'Doc':<30} {'BM25':>8} {'TF-IDF cos':>10}")
for i, doc in enumerate(docs):
    print(f"  D{i+1}: {doc:<26} {bm25[i]:>8.3f} {tfidf_scores[i]:>10.3f}")
```
...
**1. Always fit vectorizer on training data only.**
`TfidfVectorizer.fit_transform(X_train)` then `TfidfVectorizer.transform(X_test)`. If you fit on all data first, IDF values reflect test document frequencies — data leakage. Put the vectorizer inside a `Pipeline` so sklearn's cross-validation handles this correctly.

**2. max_df can remove domain-specific terms.**
Setting `max_df=0.95` removes terms appearing in >95% of documents. In a specialized corpus (e.g., all medical discharge summaries), "patient" might appear in 98% of documents and get removed — but it's still important for downstream tasks. Tune max_df per domain.

**3. Sublinear TF reduces the impact of repeated terms.**
`sublinear_tf=True` replaces TF with 1+log(TF). In our corpus, "mat" in D3 has TF=0.75. With sublinear: TF_sub = 1+log(3/4)... no wait, it applies to raw counts: TF_sub(mat,D3) = 1+log(3) = 1+1.099 = 2.099 vs raw TF count = 3. This is a soft alternative to BM25 saturation.

**4. Sparse matrix — never call .toarray() on large matrices.**
A 100K document × 50K feature TF-IDF matrix: 100K×50K×8 bytes = 40GB in dense format. As scipy sparse matrix with ~1% non-zeros: 400MB. Keep it sparse. sklearn's LogisticRegression, LinearSVC, and NaiveBayes all accept sparse matrices natively.

**5. BM25 and TF-IDF IDF formulas differ — don't mix them.**
The standard IDF = log(N/df) can be zero (when df=N). BM25's IDF = log((N-df+0.5)/(df+0.5)+1) is always positive. sklearn uses smoothed IDF = log((1+N)/(1+df))+1. These give different values — always know which formula your library uses.

**6. Cosine similarity requires non-zero vectors.**
If a document has no vocabulary words (all terms are OOV or stopped), its TF-IDF vector is all zeros → cosine similarity is undefined (division by zero). Handle with: `if norm > 0: cosine = dot/norm else: cosine = 0`.

**7. n-gram vocabulary size explodes.**
Unigrams: V features. Bigrams: up to V² features. Trigrams: V³. For V=10K: trigrams = 10¹² potential features. Always set `max_features` limit (e.g., 50K-100K). sklearn selects top features by document frequency.
...
A: sklearn's `TfidfVectorizer` applies L2 normalization by default (`norm='l2'`). After L2 normalization, cosine similarity = dot product (since ‖v‖=1 for normalized vectors). This is slightly faster. But for classification with LinearSVC or LogisticRegression, the raw (unnormalized) TF-IDF sometimes performs better because the magnitude carries information (D3 has more "mat" than D1). Try both with cross-validation.

---

## 13. Connections

**To `01_text_preprocessing.md`:**
- Preprocessing runs BEFORE vectorization. The output of preprocessing (cleaned, tokenized text) is what BoW/TF-IDF receives as input
- Stop word removal matters more for TF-IDF than for BERT — a removed stop word has zero IDF anyway, but explicitly removing it speeds up the vectorizer

**To `../embeddings/01_word_embeddings.md`:**
- TF-IDF: "cat" and "feline" → completely different columns, cosine=0
- Word2Vec/GloVe: "cat" and "feline" → similar dense vectors, cosine≈0.85
- TF-IDF is the starting point; word embeddings solve its semantic limitation

**To `../embeddings/02_word2vec_end_to_end.md`:**
- The co-occurrence matrix in GloVe is the same idea as TF-IDF's document-term matrix — except GloVe counts word-word co-occurrences, not word-document co-occurrences
- BM25 saturation ≈ GloVe's log(X_ij) transformation — both reduce the impact of very high counts

**To `../applications/01_text_classification.md`:**
- TF-IDF + Logistic Regression = the baseline to beat for text classification
- BM25 = the baseline for document retrieval

**To BERT (`5.transformers/models/05_bert_end_to_end.md`):**
- BERT produces contextual dense representations — TF-IDF's semantic limitation is fully solved
- But BERT's output for classification (the [CLS] vector) can be used as input to Logistic Regression — the same final step as TF-IDF
- The pipeline is: TF-IDF → LogReg (fast, interpretable) vs BERT → LogReg (slow, accurate)
