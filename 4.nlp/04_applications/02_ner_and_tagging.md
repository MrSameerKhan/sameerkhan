...
# Labels:          B-PER  I-PER        B-PER  -100    -100  (ignored)
    ...
    for token, label in zip(tokens, predictions):
        if label.startswith('B-'):
            ...
            if label[2:] == current_entity['type']:
                ...
                entities.append(current_entity)
                current_entity = {'type': label[2:], 'text': token, 'tokens': [token]}
        else:
            if current_entity:
                entities.append(current_entity)
            current_entity = None
...
# B-PER: precision=1.00, recall=1.00, f1=1.00
```

---

## 4. Modern NER — Open Set, Zero-Shot, LLM-Based (2023-2025)

Classical BERT-NER requires a fixed entity-type vocabulary baked into a fine-tuned classifier head. Modern alternatives lift this constraint — define the entity types at inference time.

| Approach | How | When it wins |
|----------|-----|--------------|
| GLINER (2023) | Span-classification model trained on diverse NER datasets; takes entity-type labels as input at inference | Open-set NER with thousands of types, no fine-tuning needed; runs locally; competitive with GPT-4 on many domains |
| UniversalNER (2024) | Distilled from GPT-3.5/4 for fully open-set extraction | Strong open-set; LLaMA-based; runs on a single GPU |
| NuNER / NuExtract (2024) | Pretrained encoder for arbitrary entity types | Document key-value extraction |
| LLM + function calling | GPT-4 / Claude / Llama-3.1 with structured output | Maximum flexibility; handles complex compositional entities; cost/latency tradeoff |
| LLM + GLINER hybrid | LLM for proposing types, GLINER for extraction | Best of both — uses LLM for reasoning, local model for scale |

```python
# GLINER — zero-shot NER over arbitrary types
from gliner import GLiNER

model = GLiNER.from_pretrained("urchade/gliner_multi-v2.1")
text = "Acme Corp's CFO John Smith signed the $5M deal in Dubai last Tuesday."
labels = ["company", "person", "job_title", "money", "location", "date"]
entities = model.predict_entities(text, labels, threshold=0.5)
# [{"text": "Acme Corp", "label": "company"}, {"text": "John Smith", "label": "person"}, ...]

# LLM with structured output (Instructor + Pydantic) — most flexible
...
result = client.chat.completions.create(
    ...
    messages=[{"role": "user", "content": f"Extract PERSON, ORG, MONEY entities from: {text}"}],
)
```

**Senior interview answer in 2025:** "For a fixed entity schema with ≥1000 labeled examples, fine-tuned BERT-NER is still the cheapest production option (low latency, low cost). For open-set extraction with thousands of possible types, or when labels evolve weekly, I'd reach for GLINER (local) or LLM + function calling."
...
def span_overlap_f1(true_spans, pred_spans):
    ...
    tp = sum(len(set(range(*s)) & set(range(*p)))
...
**Sentence boundaries matter:** BERT processes sentences, not documents. Split at sentence boundaries before tokenization. A 512-token window arbitrarily split mid-sentence confuses the model.
...
**Low recall on long entity spans:** Increase max_length (default 128 is often too short for NER). Check if spans are truncated during tokenization. Use stride/overlap for long documents.

**Model predicts I-TYPE without B-TYPE:** IOB constraint violation — model doesn't understand tag structure. Solution: add CRF head on top of BERT token logits. Or post-process: convert invalid I-TYPE at sequence start to B-TYPE.

**Poor performance on rare entity types:** Per-type F1 analysis first; seqeval: metrics: classification_report. Augment with entity replacement (swap all PER entities with random names from gazetteer). Increase class weight for rare types in loss.

**Training loss goes to 0 but eval F1 stays low:** Overfitting to training labels — add more dropout (0.2-0.3). Check for label noise — NER annotation is expensive and often inconsistent. Use span-level training with stronger data augmentation.
...
A: A CRF (Conditional Random Field) is a discriminative sequence model that models the conditional probability P(y|s) of a label sequence given observations. Unlike classifying each token independently, CRF uses the Viterbi algorithm to find the globally optimal tag sequence, incorporating label transition probabilities. This means it learns that B-PER → I-PER is valid but B-PER → I-ORG is not. Pre-BERT, BiLSTM-CRF was state-of-the-art because the BiLSTM captures context and the CRF enforces valid IOB transitions.
