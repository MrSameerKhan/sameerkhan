...
| OpenAI `text-embedding-3-small` | Black-box API | 1536 | API latency | Strong general purpose |
...
OR: Multiple Negatives Ranking (MNR) loss (better for retrieval):
  For batch of (query, positive) pairs → all other positives are negatives
...
| `multi-qa-MiniLM-L6-cos-v1` | 22M | 384 | ~14K sent/s | Q&A retrieval specifically |
...
# D: cosine scores, I: document indices
...
| Pre-computation | ✓ index docs offline | ✗ must see both at query time |
...
# Multiple Negatives Ranking Loss: other positives in batch = negatives
...
**MNR loss works best with large batch sizes** (more negatives = harder negatives = better training signal). Use batch_size=64+ when possible.
...
**Q: Why is SBERT better than BERT for semantic similarity?**
A: BERT's [CLS] token and mean-pool without fine-tuning produce embeddings where cosine similarity doesn't correlate with semantic similarity (STS-B ≈ 0.59). SBERT fine-tunes with a siamese setup on NLI/STS data so that semantically similar sentences are close in the embedding space — STS-B ≈ 0.86+. The key change is the training objective: BERT's MLM doesn't optimize for similarity structure; SBERT's contrastive/cosine loss directly does.

**Q: When do you use a cross-encoder instead of a bi-encoder?**
A: Bi-encoder when you need pre-computable embeddings: semantic search over millions of documents (encode corpus offline, search at query time in < 5ms). Cross-encoder when quality matters more than speed: reranking top-K candidates, where you can afford 100–200ms. Standard pipeline: bi-encoder retrieves top-100, cross-encoder reranks to top-5.

**Q: How does Multiple Negatives Ranking Loss work?**
A: For a batch of (query, positive) pairs, each query's positives in the batch become negatives for other queries. Loss = cross-entropy where the positive gets score exp(cos(q,p)/τ) and the negative sum is Σ exp(cos(q,nᵢ)/τ). With batch_size=64, each query has 63 negatives — no need to manually label negatives. Larger batch → harder negatives → better model.
...
| Cross-encoder reranking | `9.system_design/03_search_and_rag_system.md` |
| RAG with sentence embeddings | `6.llms/08_rag_end_to_end.md` |
