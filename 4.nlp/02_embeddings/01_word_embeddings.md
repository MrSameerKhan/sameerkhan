...
**2024-25 production note:** For new retrieval / similarity / RAG work, skip static word embeddings entirely and start with a contextual sentence embedder (`02_sentence_embeddings.md`) — BGE, E5, Nomic, text-embedding-3) or a BERT-style encoder. Static embeddings are still useful for: very fast baselines on huge corpora (Word2Vec is ~1000× cheaper than BGE), morphology-rich low-resource languages (FastText handles OOV via subwords), and as a teaching tool for understanding how dense representations emerged. **The concepts below — co-occurrence, softmax-via-negative-sampling, vector arithmetic — still underpin modern contrastive embedding training.**
...
```
Sparse (TF-IDF):
  "cat"    → [0, 1, 0, 0, ..., 0]  (50K-dim, one-hot style)
  "feline" → [0, 0, 0, 0, 1, ..., 0]
...
Why "cats" ≈ "fish" (cosine 0.99 here)? Both appear in the same context (after "love", near each other). During training, their rows get pulled toward the same direction. "love" lives in a different region (high dim_1, low dim_0) — it plays a syntactic role (verb), not a noun.
...
print(s2.shape)  # (3, 3) → [seq_len=3, embed_dim=3]
print(s2)
# [[ 0.89 -0.32  0.76]
#  [ 0.05  0.92  0.10]
#  [ 0.83 -0.28  0.71]]
...
**Key insight:** The embedding matrix is just a lookup table. Index in → row out. Training (Word2Vec / GloVe / fine-tuning) is simply adjusting these rows so semantically similar words end up with similar row vectors.
...
GloVe trains embeddings such that `w_i · w_j = log(X_ij)`. The dot product between embeddings should reflect log co-occurrence count.

### GloVe Loss Function

```
J = Σ_{i,j} f(X_{i,j}) · (w_i · w̃_j + b_i + b̃_j - log X_{i,j})²

f(X_{i,j}) = weighting function:
  f(x) = (x/x_max)^α if x < x_max, else 1
  Downweights very frequent co-occurrences (stop words)
  x_max = 100, α = 0.75 (standard)

w_i = word vector, w̃_j = context vector, b_i, b̃_j = biases
```
...
glove.6B.300d   — Same corpus, 300 dimensions (larger, better)
...
Word2Vec / GloVe:
  ...
  "running" (typo) → OOV → zero vector or [UNK] ❌
...
print(model.wv['runing'])     # typo — still works via shared n-grams!
...
# Pairs seen: (love,cats)×2, (cats,love)×1, (fish,love)×1
# Result: "cats" and "fish" end up similar because both co-occur with "love"
w2v = {
    "I":     np.array([ 0.10,  0.00, -0.20]),
    "love":  np.array([ 0.05,  0.92,  0.10]),
    "cats":  np.array([ 0.89, -0.32,  0.76]),  # + similar to cats
...
glove = {
    "I":     np.array([ 0.12,  0.79, -0.18]),
    "love":  np.array([ 0.04,  0.93,  0.09]),
    "cats":  np.array([ 0.88, -0.31,  0.74]),  # + slightly further from cats
    "fish":  np.array([ 0.70, -0.18,  0.60]),  # + because X[love][fish] < X[love][cats]
}
...
| love-cats bond > love-fish bond? | ❌ depends on sampling | ✅ yes (count=2 vs 1) | ✅ yes |
| "catss" (OOV typo) | ❌ zero vector | ❌ zero vector | ✅ ~cats |
| "fishing" (OOV form) | ❌ zero vector | ❌ zero vector | ✅ ~fish |
| Training requires full corpus upfront? | streaming pairs | full count matrix | streaming pairs + n-grams |
...
def load_pretrained_embeddings(vocab, glove_path, dim=300):
    ...
    with open(glove_path) as f:
        for line in f:
            ...
            word = parts[0]
            vector = np.array(parts[1:], dtype=np.float32)
            glove[parts[0]] = vector

    # Initialize with small random (for words not in GloVe)
...
sentence_vec = np.mean(word_vectors)  # loses word order
```
...
Static embeddings (Word2Vec, GloVe, FastText):
  "bank" (river)   → [0.23, -0.45, 0.78, ...]  → SAME vector!
...
| Custom domain vocabulary | Train Word2Vec/FastText on domain | Domain-specific embeddings |
...
**Static embeddings can't handle polysemy.** "Apple" (company) and "apple" (fruit) get the same vector. For tasks where word sense matters, you need contextual embeddings (BERT).

**Out-of-vocabulary at inference must be handled.** For Word2Vec/GloVe: if a word is OOV at inference, you have no vector. Either skip it (mean of other words), use UNK vector (mean of all training vectors), or switch to FastText.

**Embedding dimension affects model size significantly.** 300d embeddings with 100K vocabulary = 30M parameters just for the embedding layer. If model is memory-constrained, use 50d or 100d embeddings.

**Don't compare embedding trained on different corpora.** Word2Vec trained on medical text and GloVe trained on news have different vector spaces — cosine similarities between them are meaningless. Always use embeddings from the same model.

**Mean pooling loses word order.** "man bites dog" and "dog bites man" have the same mean embedding. If order matters (it almost always does), use LSTM/Transformer over word embeddings, not just mean pooling.
...
The critical insight: Word2Vec encodes "what words appear near each other" —
semantics emerge. This is why king − man + woman ≈ queen — the male/female
difference is encoded linearly in the embedding space from co-occurrence patterns.
```
