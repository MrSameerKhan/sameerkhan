...
#  [0.821, 1.000, 0.090, 0.912],   ← "bill not paid" — close to others
#  [0.102, 0.090, 1.000, 0.089],   ← "pizza" — unrelated to others
...
Objective 1 — Regression (STS tasks):
  L = MSE(cosine(emb_a, emb_b), gold_similarity)
...
# 8.76:  The payment is past due date
...
**Domain mismatch.** General-purpose embeddings (trained on Wikipedia, web) may underperform on specialized domains (medical, legal, code). Fine-tune on in-domain data or use domain-specific models (e.g., BioBERT for biomedical).
...
(1) Offline: encode all 1M documents with a bi-encoder (e.g., all-mpnet), build a FAISS index (HNSW for ANN search). (2) Online: encode query → FAISS.search(query, k=100) → retrieve 100 candidates in sub-millisecond. (3) Rerank: run cross-encoder on (query, 100 candidates) → re-sort by score → return top-10. (4) Generate: pass top-10 to LLM as context. Total latency: ~200ms. Scale: FAISS handles 1M 768-dim vectors on 6GB RAM.

---

## 2024-25 Production Retrieval Stack (Hybrid + RRF + Reranker)

Modern production retrieval combines three signals:

```
Query
  ├── BM25 (sparse, exact-match) ─────────────── top-50 sparse
  ├── Dense embedding (BGE / E5) ─────────────── top-50 dense
  │
  └── Reciprocal Rank Fusion (RRF) ──────────── merged top-50
        │
        └── Cross-encoder reranker ──────────── final top-5 → LLM context
              (bge-reranker-v2, Cohere Rerank, jina-reranker)
```

Why all three:
- BM25 catches **exact-term matches** dense models miss (acronyms, codes, rare names)
- Dense catches **paraphrases and concepts** BM25 misses
- RRF fuses rankings without tuning weights

```
RRF score: 1/(k + rank) summed across rankers) — Reranker uses full query-passage attention on the final 50 — quality bi-encoder can't match alone
```

Hybrid + reranker typically beats either alone by **5-15% nDCG@10** on BEIR. Modern dense-embedder choice matters too: see `02_sentence_embeddings.md` for BGE, E5, Nomic, mxbai, jina-v3, text-embedding-3 comparison.

```python
# Hybrid retrieval with RRF
def hybrid_search(query, bm25_index, dense_index, encoder, k=50, rrf_k=60):
    bm25_topk   = bm25_index.search(query, k)         # list of doc_ids
    q_emb       = encoder.encode([query], normalize_embeddings=True)
    dense_topk  = dense_index.search(q_emb, k)[1][0].tolist()

    scores = {}
    for rank, doc_id in enumerate(bm25_topk):
        scores[doc_id] = scores.get(doc_id, 0) + 1 / (rrf_k + rank + 1)
    for rank, doc_id in enumerate(dense_topk):
        scores[doc_id] = scores.get(doc_id, 0) + 1 / (rrf_k + rank + 1)
...
- Sentence embeddings end-to-end: `03_sentence_embeddings_end_to_end.md`
- Production RAG ops: `../../10.mlops/13_production_rag_ops.md`
...
- `code_practice/05_rag/03_vector_db/` — FAISS + Chroma
- `code_practice/05_rag/06_hybrid_search/` — BM25 + dense + RRF
