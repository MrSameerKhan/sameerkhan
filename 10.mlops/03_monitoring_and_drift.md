...
> A model that performs well in development will degrade in production — when and by how much is unknowable without monitoring.
...
def ks_test_drift(reference: np.ndarray, current: np.ndarray, threshold: float = 0.05):
    """Kolmogorov-Smirnov test for continuous distribution shift."""
...
class PredictionMonitor:
    ...
    def _check_drift(self):
        ...
        # 1. Prediction distribution (KS test)
        ...
        if mean_confidence < 0.8:  # threshold from calibration analysis
...
Retraining data strategies:
  ...
  2. Sliding window: use only last N months of data
...
**Distribution shift ≠ performance degradation:** A feature distribution can shift significantly while model performance stays the same (the shift is in an unimportant feature). Don't over-alert on drift — correlate drift with performance signals before taking action.
...
Data drift: the distribution of input features changes after deployment — model was trained on one distribution but is being served a different one. Detection: (1) PSI — bins the distribution and compares proportions; PSI > 0.2 signals significant drift. (2) KS test — compares CDFs of reference vs current; p-value threshold for continuous features. (3) Chi-squared test for categorical features. In practice: compute these daily and route to a dashboard (Grafana + Prometheus or Evidently).

**Q: What is the difference between data drift and concept drift?**

Data drift: P(X) changes — different types of queries, different document formats, different user demographics. The model might still be correct if the drift is in irrelevant features. Concept drift: P(Y|X) changes — the correct answer for the same input changes. Example: post-COVID, "getting sick" had negative sentiment but "getting tested" positive. Concept drift requires retraining; data drift may not. Concept drift requires delayed ground truth labels, which is why delayed label collection infrastructure is critical.
...
- MLOps Serving (`7.mlops/02`): Latency/throughput metrics from the serving layer
- MLOps Pipelines (`7.mlops/04`): Automated retraining triggered by monitoring alerts

## Key Takeaway

```
Monitor in four layers: infrastructure (latency, errors), data quality (missing values, schema),
model behavior (prediction distribution, confidence), and business KPIs.
Data quality + PSI > 0.2 = early warning signal before labels arrive.
Alert on: AUC regression > 3%, PSI > 0.20, prediction rate shift > 5%.
Calibrate threshold when AUC is stable but calibration shifted.
Retrain when performance drops or drift is critical; retraining is the hardest part.
Ground truth latency is the hardest problem — can only detect input drift in real time.
```
