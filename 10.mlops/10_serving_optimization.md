# 10 — Serving Optimization — End to End
...
Problem: GBM classifier fine, but PyTorch model deployed for invoice classification.
  ...
  Serving requirement: <20ms p95 C1000 QPS, real-time API
...
def benchmark(model_fn, input_data, n_warmup=50, n_runs=500):
    ...
    device = next(model_fn.parameters()).device if hasattr(model_fn, 'parameters') else 'cpu'
...
x_np = torch.randn(1, 128).cuda()
baseline = benchmark(model, x_np)
...
ort_out = ort_session.run(["logits"], {"features": x_np})[0]
...
# {'p50_ms': 2.3, 'p95_ms': 3.4, 'p99_ms': 6.1, 'mean_ms': 2.2, 'throughput': 455.0}
...
print("PyTorch Dynamic INT8:", dynamic_metrics)
...
# {'p50_ms': 1.7, 'p95_ms': 2.3, 'p99_ms': 2.3, 'mean_ms': 1.2, 'throughput': 833.0}
```
...
| ONNX FP32                | 2.3ms   | 3.4ms   | 6.1ms   | 455 req/s   | 12.2 MB (3.6× faster) |
...
print(f"AUC drop: {(fp32_auc - int8_auc)*100:.3f}%  (required: <0.1% threshold)")
# FP32 AUC: 0.9231
# INT8 AUC: 0.9218
...
# 80GB GPU → only 160 concurrent requests → saturates fast
...
prompts = [
    "Extract the invoice number from: INV-2024-0032, dated March 16...",
...
Traditional KV cache:
  ...
  Request 2: needs 12MB but only 400MB available → rejected even though
             request 2 only uses 100 tokens (100MB actual)

PagedAttention (like OS virtual memory paging):
  KV cache divided into fixed-size "pages" (16 tokens = ~2MB each)
  Request 1 gets pages 0,1,2... as needed (no pre-allocation)
  Request 3 gets pages H0,H1,H2... from available pool
...
Quantization type?
  ...
  Latency critical (< 8ms):            static quantization or ONNX INT8 (needs calibration)
...
**Batch size matters for throughput, not latency.** Single-item latency (online serving) improves with ONNX/INT8. Throughput (batch serving) improves dramatically with larger batches. Measure both: p95 for SLA, throughput for capacity planning.
...
**PagedAttention math.** KV cache per token for Mistral-7B: 2 (K+V) × 32 layers × 8 heads × 128 head_dim × 2 bytes (FP16) = 131KB per token. 4096 token context = 512MB per request. This is why LLM serving is memory-bound, not compute-bound.
...
(1) Baseline: measure p50/p95 latency and throughput with representative inputs. (2) ONNX export: `torch.onnx.export()` with dynamic axes, verify outputs match within floating point tolerance. (3) ONNX Runtime: typically 3-4× speedup over PyTorch on CPU without any accuracy loss. (4) Quantization: dynamic INT8 for quick wins, static INT8 for maximum speedup. (5) Validate: measure AUC/accuracy after each step — reject if drop > 0.1%. (6) Serve with FastAPI + ORT session configured with optimal thread counts.

**Q: What is PagedAttention and why does it matter?**

Standard LLM serving pre-allocates a contiguous KV cache block for each request sized for max sequence length. If max_seq_len=4096 but you serve 200 users, 512MB × 200 = all allocation is wasted — fragmentation means you can serve far fewer concurrent requests than the GPU memory would allow. PagedAttention stores KV cache in small fixed-size pages (like OS virtual memory paging). Pages are allocated on demand and can be non-contiguous. Result: near-zero fragmentation, 3-5× more concurrent requests, same GPU, same latency — massive throughput improvement at the same cost.

**Q: What is the tradeoff of INT8 quantization?**

INT8 uses 1 byte per weight vs FP32 (4 bytes) or FP16 (2 bytes). Benefits: 2-4× size reduction, 2-7× speedup (faster memory bandwidth, INT8 multiply-accumulate is faster than FP ops on modern CPUs). Cost: 8-bit representation has 256 values vs FP32's ~16M — small rounding errors accumulate. In practice for most models: AUC drop < 0.1%, which is acceptable. For models with sparse activations or large dynamic range, accuracy can drop more — always validate.

---

## Connections

- Model registry (`8.mlops/08`): track optimized model versions
- Serving and inference reference (`8.mlops/02`): vLLM, FastAPI details
- Monitoring (`8.mlops/09`): monitor latency after deployment

## Key Takeaway

```
Optimization pipeline: PyTorch → ONNX → INT8 gives 5-10× speedup
with <0.1% accuracy loss on CPU. Always measure baseline first,
validate accuracy after each step, never ship without benchmarking.

For LLMs: vLLM (PagedAttention + continuous batching) gives 3-5×
throughput improvement by eliminating KV cache fragmentation.

Standard path: ONNX INT8 for sklearn/small neural nets; vLLM +
AWQ 4-bit for LLMs.

Rule: measure → optimize → validate → repeat.
```
