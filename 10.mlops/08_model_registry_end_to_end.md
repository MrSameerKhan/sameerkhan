...
with mlflow.start_run(run_name="gbm-depth5-lr0.05") as run:
    ...
    metrics = {
        ...
        "val_f1":       f1_score(y_val, y_pred_proba > 0.42),
...
print("Model registered as 'invoice-classifier' version 1")
```
...
# d3e295a...      4       0.03    0.915    0.862
# fa6c8da...      5       0.10    0.908    0.854
# ...
...
# Log promotion event
client.set_model_version_tag("invoice-classifier", 5, "promoted_at", "2024-08-19")
client.set_model_version_tag("invoice-classifier", 4, "promoted_by", "sameer")
client.set_model_version_tag(
    "invoice-classifier", 5, "promotion_reason",
...
with mlflow.start_run():
    ...
    mlflow.pytorch.log_model(
        ...
        conda_env={"dependencies": ["torch==2.0", "numpy"]},
    )
```
...
**Never load by version number in serving.** `models:/name/Production` decouples serving from registry ops. If you hardcode version 3, version 4 doesn't take effect automatically.

**Artifact path matters.** `mlflow.sklearn.log_model(..., artifact_path="model")` sets the path within the run. Loading `models:/name/Production` resolves to `artifact_path="model"`. Inconsistent paths across runs → confusion when loading.

**Schema drift.** Model trained on 128 features; production sends 130 features. Log `input_example` and add a schema check in your CI gate: `signature` when logging — MLflow will validate at serving time. Add schema check in CI gate.
...
A model registry is a versioned catalog of ML model artifacts with associated metadata (metrics, parameters, training data, git commit). You need it because: (1) reproducibility — know exactly what code/data produced the production model; (2) governance — who approved it, when, why; (3) rollback — immediately restore the previous version if production degrades; (4) team coordination — multiple engineers can train models without overwriting each other. Without a registry, you end up with model.pkl files on someone's laptop and no way to audit or roll back.

**Q: What checks should a staging validation gate include?**

(1) Functional metrics on holdout test set — must meet or beat a threshold (e.g., AUC > 0.91); (2) regression check — new model must not be worse than current production by more than a margin; (3) latency test — p95 inference time under budget; (4) schema validation — accepts expected input shapes and types; (5) edge case tests — known hard examples should pass; (6) data drift check — training distribution should match expected serving distribution.

**Q: How do you roll back a model in production?**

In MLflow: transition the previous version (now Archived) back to Production stage. The serving layer loads by `models:/name/Production`, so it picks up the rollback automatically — no code change needed. The key is that serving code must reference stage, not version number. With a 60-second model reload interval, rollback takes effect within 1 minute.

---

## Connections

- Experiment Tracking (`8.mlops/01`): MLflow runs, metrics, artifacts
- Serving and Inference (`8.mlops/02`): loading model, FastAPI, vLLM
- Monitoring (`8.mlops/09`): detecting when production model degrades
- CI/CD Pipelines (`8.mlops/04`): how validation gate integrates with GitHub Actions

## Key Takeaway

```
Model registry = train → log (metrics + artifact) → register → stage (Staging)
→ validate (CI gate: AUC + latency + schema) → promote (Production) → monitor
→ rollback if needed. Always load by stage (Production) not version number —
that's what enables zero-code-change rollbacks. MLflow is the standard
open-source tool; SageMaker Model Registry and Vertex AI Model Registry are
cloud equivalents.
```
