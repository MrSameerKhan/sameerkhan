...
| LLaMA 2 | 7B-70B | 4096 | Open weights, efficient |
| LLaMA 3 | 8B-70B | 128K | Best open model |
...
During inference: sequential, one token at a time
  Generate xₜ → append to context → generate xₜ₊₁ → ...
```

**Why causal masking during training:**
```
Upper-triangular mask on attention scores prevents position i from
attending to positions j > i. This allows training on all positions
simultaneously while enforcing left-to-right dependence.
...
### GPT-1 → GPT-3 Evolution

**GPT-1 (2018):**
```
117M params, 12 layers, d_model=768, 12 heads
512 context length, 4GB BookCorpus data
...
**LLaMA 2 Architecture improvements over GPT:**
```
...
KV cache memory: O(n · L · 2 · d_model) with MHA
                 O(n · L · 2 · d_model/4) with GQA (g=h/4)
...
# Temperature sampling (T<1 = sharper, T>1 = more random)
...
```
Greedy: P(xₜ) = argmax P(xₜ | context)
  ✓ Fast, deterministic
  ✗ Repetitive, misses high-probability longer paths

Beam Search: maintain B partial sequences at each step, prune to B best
  ✓ Better than greedy for translation, summarization
  ✗ Tends toward generic/safe text for open-ended generation

Temperature: scale logits before softmax: P(x) ∝ exp(logit(x)/T)
  T→0: greedy, T=1: original distribution, T→∞: uniform
  ✓ Simple and effective
  ✗ Can still produce low-quality tail tokens

Top-k: P(x) = 0 for all x not in top k tokens
  ✗ Fixed k inappropriate when distributions vary in sharpness

Top-p (nucleus): dynamic cutoff based on cumulative probability
  ✓ Adapts to distribution sharpness
  ✓ Best for creative generation in practice
...
# LLaMA 7B: 14GB (fp16) → 3.5GB (4bit) — fits on single consumer GPU
...
- **Encoder-Decoder (models/03):** T5/BART — hybrid for seq2seq tasks
