...
| Sparse Attention | O(n²) for long sequences | Misses some token interactions |
| DistilBERT / knowledge distillation | Model too large | Slight accuracy drop |
| LoRA / QLoRA | Full fine-tuning too expensive | Minor quality gap vs full fine-tune |
| Quantization (INT8/4-bit) | Inference too slow / large | Small accuracy drop |
| Speculative Decoding | Generation too slow | Requires draft model |
| MoE | Scale without compute cost | Expert routing overhead |
...
Standard attention (simplified):
  S = QKᵀ                          # [n, n] — write to HBM
  P = softmax(S / √dₖ)             # [n, n] — write to HBM
...
# PyTorch scaled_dot_product_attention (uses Flash Attention when available)
...
Combines three attention patterns:
  ...
  2. Local window: each token attends to w neighbors (O(n·w))
...
For a pretrained weight matrix W₀ ∈ ℝ^(d×k):
  Standard fine-tuning: update W = W₀ + ΔW  (d×k parameters)
  LoRA: ΔW = BA  where B ∈ ℝ^(d×r), A ∈ ℝ^(r×k), r << min(d,k)

Parameters: d×k → r×(d+k), e.g., d=1024, k=1024, r=8: 1048576 → 16384 (64× less)

During forward pass:
  h = W₀x + (BA)x = W₀x + (scale · B · A · x)
...
Optional: Wk, Wo, FFN projections
```
...
Router(x) → select top-k experts → weighted sum of their outputs

FFN_MoE(x) = Σᵢ∈top_k gate(x)ᵢ · Expert_i(x)
...
| Scale without compute | MoE (Mixtral) | More params, same FLOPS |
...
**Merge LoRA before serving:** After training, merge `W = W₀ + BA` to get a single weight matrix. No runtime overhead. Forgetting to merge → 2 matrix multiplications instead of 1.
...
- **GPT Family (models/02):** LoRA/QLoRA primarily used for LLM fine-tuning; Mixtral is a GPT-style MoE
