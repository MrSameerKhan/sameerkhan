...
```
GPT-2 (2019)  →  LLaMA-1 (2023)  →  LLaMA-2 (2023)  →  Mistral (2023)  →  LLaMA-3 (2024)
```

GPT-2 established the transformer decoder baseline.  
Every model since then is GPT-2 + a set of targeted improvements.
...
| Position encoding | Learned absolute | **RoPE** | Relative positions, length generalization |
| Attention | MHA | **GQA** (LLaMA-2+) | KV-cache memory savings |
...
Residual is computed first, then normalized.  
Problem: gradients must flow through the LayerNorm — can cause instability in deep networks.
...
Normalize first, then compute attention, then add residual.  
Gradient flows through the residual stream unmodified — no normalization in the gradient path.
...
GPT-2 learns one embedding vector per position: `PE_0, PE_1, ..., PE_1023`.  
Added to token embeddings at the start.

Problems:
1. Fixed at train time — can't generalize beyond 1024 tokens without retraining
2. Absolute positions — the model learns "token at position 5" not "token 3 steps before current"
3. Q·K dot product mixes position and content in a hard-to-separate way

### RoPE Key Idea

Don't add position to embeddings. Instead, **rotate** Q and K vectors by an angle proportional to position before computing attention.

The rotation angle is position-dependent, so Q·K naturally encodes relative position.

### Math Setup (d=2 for clarity)

For 2D queries/keys, define angle:
```
θ = 1.0 (base angle for d=2, one pair of dimensions)
```

For position m, the rotation is:
```
...
Let q = k = [1, 0] (simplest unit vector).

After rotation:
```
q_rot at position m = [cos(m), sin(m)]
k_rot at position n = [cos(n), sin(n)]
...
Fast θ → high-frequency rotation → encodes short-range positions  
Slow θ → low-frequency rotation → encodes long-range positions (like sinusoidal PE)
...
q_rot[2] = q_2×cos(0.02) - q_3×sin(0.02) = 0.8×0.9998 - 0.3×0.0200 = 0.800 - 0.006 =  0.794
q_rot[3] = q_2×sin(0.02) + q_3×cos(0.02) = 0.8×0.0200 + 0.3×0.9998 = 0.016 + 0.300 =  0.316
```
...
Sinusoidal and learned absolute embeddings require you to have seen position 2000 during training to handle sequences of length 2000.

RoPE embeds position as a rotation angle — the model learns to handle relative angles, not absolute positions. Rotating by a larger angle (longer sequence) is an extrapolation of the same operation.
...
The gate `x × W_3` controls how much of the activation `Swish(x × W_1)` passes through.  
If the gate is near 0 → block the activation. If near 1 → let it through.
...
Swish(act) = act × σ(act) = 0.75 × σ(0.75)
  σ(0.75) = 1 / (1 + e^{-0.75}) = 1 / (1 + 0.472) = 1 / 1.472 = 0.680
...
GELU is fixed activation — no data-dependent control.  
SwiGLU: the gate W_3×x is learned and depends on input x.  
Network can learn to suppress certain features for certain inputs.
...
During inference, at each new token, we compute:
- Q for the new token
- K, V for the new token → appended to the cache
- Attention over all cached K, V

**KV cache size** = (sequence length) × H × d_k × 2 (K and V)
...
GQA (Grouped Query Attention) — LLaMA-2, LLaMA-3, Mistral:
  ...
  H=4, G=2: Q_1,Q_2 share K_1,V_1 | Q_3,Q_4 share K_2,V_2
...
```
Token 1: K_1=[0.5,0.3], K_2=[0.4,0.1], K_3=[0.6,0.2], K_4=[0.3,0.4]
         V_1=[0.7,0.2], V_2=[0.5,0.3], V_3=[0.8,0.1], V_4=[0.2,0.6]

Token 2: K_1=[...], K_2=[...], K_3=[...], K_4=[...]  ← 4 K vectors per token
         V_1=[...], V_2=[...], V_3=[...], V_4=[...]  ← 4 V vectors per token
...
Token 1: K_g1=[0.5,0.3], K_g2=[0.6,0.2]   ← only 2 K vectors (was 4)
         V_g1=[0.7,0.2], V_g2=[0.8,0.1]   ← only 2 V vectors (was 4)
...
Scores against cached K_g1 (all 3 tokens):
  ...
  score_2 = q_1 · K_g1[token2] = 0.8×0.3 + 0.6×0.4 = 0.24 + 0.24 = 0.48
  score_3 = q_1 · K_g1[token3] = 0.8×0.1 + 0.6×0.5 = 0.08 + 0.30 = 0.38

Softmax([0.58, 0.48, 0.38]):
  e^0.58 = 1.786, e^0.48 = 1.616, e^0.38 = 1.462
  sum = 4.864
  α = [0.367, 0.332, 0.301]
...
Standard attention: token at position t can attend to all positions 0..t.  
Cost: O(T²) for a sequence of length T.

Mistral's Sliding Window Attention: each token attends to only the last W positions.  
Cost: O(T × W) — linear in T for fixed W.
...
def rotate_vector(q, position, angles):
    ...
    for i, theta in enumerate(angles):
        ...
        q_rot[2*i]   = q[2*i] * cos_a - q[2*i+1] * sin_a
        q_rot[2*i+1] = q[2*i] * sin_a + q[2*i+1] * cos_a
    return q_rot
...
for (m, n) in [(1,0), (2,1), (3,2), (10,9)]:
    ...
    print(f"  pos ({m},{n}), rel={m-n}: score = {score:.4f}")
...
for (m, n) in [(1,0), (2,0), (3,0)]:
    score = rope_attention_score(q, k, m, n)
    print(f"  pos ({m},{n}), rel={m-n}: score = {score:.4f}")
# Scores change as relative distance increases
...
print("\nGELU FFN output:  ", np.round(out_gelu, 4))
print("SwiGLU FFN output:", np.round(out_swiglu, 4))
...
print(f"  MHA (32 KV heads): {mha/1e9:.2f} GB")
print(f"  GQA  (8 KV heads): {gqa/1e9:.2f} GB  ({mha//gqa}× smaller)")
print(f"  MQA  (1 KV head):  {mqa/1e9:.2f} GB  ({mha//mqa}× smaller)")
```
...
Relative position invariance:
  ...
  pos (3,2), rel=1: score = 0.6832
  pos (10,9), rel=1: score = 0.6832  ← identical ✓
...
KV Cache at seq_len=4096:
  ...
  GQA  (8 KV heads): 0.27 GB  (4× smaller)
  MQA  (1 KV head):  0.03 GB  (32× smaller)
```
...
> RoPE encodes position as a rotation angle. Attention scores depend only on relative angle (m-n), not absolute position. Adding a longer sequence just applies larger rotations — the model has seen the rotational relationship between nearby tokens, which extrapolates.  
> Absolute embeddings fail because PE_2000 was never seen if trained only up to 1024.

**Q: Why pre-norm instead of post-norm?**
> Post-norm: `LayerNorm(x + Attention(x))` — gradient must flow through LayerNorm, causing instability in deep networks.  
> Pre-norm: `x + Attention(RMSNorm(x))` — the residual stream has a clean gradient path ∂L/∂x = 1 + ..., never blocked by normalization. LLaMA-1 (65B, 80 layers) would be hard to train with post-norm.

**Q: Why does GQA save so much memory?**
> KV cache stores K, V for every token in the sequence. Each head has its own K, V in MHA.  
> GQA groups heads — G groups share K, V instead of H separate pairs.  
> LLaMA-70B: H=64 heads → G=8 groups → 8× smaller KV cache.  
> This directly enables larger batch sizes and longer sequences during inference.

**Q: What is SwiGLU and why is it better than GELU?**
> GELU: `h = GELU(W_1 x)`, then project. Fixed activation, no input-dependency.  
> SwiGLU: `h = Swish(W_1 x) ⊙ (W_3 x)`. The gate `W_3 x` is learned and input-dependent.  
> Network learns to suppress or amplify features per input. Empirically +0.5-1 point on downstream tasks.

**Q: What does Mistral's Sliding Window Attention solve?**
> Standard attention is O(T²) memory — a 32k token sequence needs T² = 1 billion attention weights per head.  
> SWA: each token attends to only the last W=4096 tokens → O(T×W). Linear in T.  
> Global info propagates via layers: after L layers, effective receptive field = L×W tokens.  
> Mistral 7B: W=4096, L=32 → 131k effective context from 4096-token windows.

**Q: How does RMSNorm differ from LayerNorm computationally?**
> LayerNorm: subtract mean, divide by std — requires mean + variance computation.  
> RMSNorm: only compute RMS (sqrt of mean of squares), no mean subtraction.  
> No β (shift) parameter. ~40% faster than LayerNorm in practice.  
> Intuition: the mean subtraction in LayerNorm centers the output; empirically centering doesn't help for transformers, so it can be removed.

**Q: Why is LLaMA-2 70B so much more practical than a hypothetical 70B MHA model?**
> MHA KV cache at seq_len=4096: ~107 GB (80 layers × 64 heads).  
> This alone exceeds 2×A100-80GB GPU memory — nothing left for weights.  
> GQA (8 KV heads): ~13 GB for KV cache + 140 GB for model weights (fp16) = 153 GB.  
> Fits on 2×A100-80GB with enough headroom for a reasonable batch size.
