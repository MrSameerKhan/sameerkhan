...
> **Used in:** GPT-4, Mixtral 8×7B, Gemini 1.5, Switch Transformer, GLaM
...
MoE model: each token is routed to only K of N expert FFN layers
  Mixtral 8×7B: 8 experts per layer, each token uses 2 experts
...
MoE Transformer FFN layer:
  ...
  Experts: E₁, E₂, ..., E₈  (each is an independent FFN: d→4d→d)
...
```
Input token: x ∈ ℝ^d   (d = 4096 for Mixtral)

Router scores:
  h(x) = Linear(d, N)(x)    ← N = 8 for Mixtral
  h(x) ∈ ℝ^8               ← 8 raw logits
...
Step 4 — Compute:
  E₃("invoice") → vector v₃ ∈ ℝ^4096
  E₆("invoice") → vector v₆ ∈ ℝ^4096

Step 5 — Weighted sum:
  output = 0.588 × v₃ + 0.412 × v₆
...
Without load balancing:
  Expert 1: 94% of tokens  ← over-trained
...
def load_balancing_loss(router_probs, expert_indices, n_experts):
    ...
    # Loss = n_experts × dot(tokens_fraction, avg_router_prob)
...
Balanced (ideal):
  tokens_per_expert  = [0.125, 0.125, ..., 0.125]   (all equal)
  router_prob/expert = [0.125, 0.125, ..., 0.125]
  aux_loss = 8 × (8 × 0.125 × 0.125) = 8 × 0.125 = 1.0  ← minimum

Collapsed:
  tokens_per_expert  = [0.94, 0.01, ..., 0.01]
  router_prob/expert = [0.85, 0.02, ..., 0.02]
  aux_loss = 8 × (0.94×0.85 + 7×0.01×0.02)
           = 8 × (0.799 + 0.0014)
           = 8 × 0.800 = 6.40  ← higher, penalized
```
...
| Property | Dense (LLaMA-7B) | MoE (Mixtral 8×7B) |
|----------|-----------------|---------------------|
...
**The catch:** MoE needs ALL experts in memory during inference, even though only K=2 are used per token. Mixtral 8×7B needs ~93GB VRAM at FP16 → requires 2× A100 80GB for inference. Quantized (4-bit): ~24GB → fits on 2× RTX 3090.
...
# Mixtral — needs 2× A100 80GB or offloading
...
# Memory: ~24GB (fits 2× RTX 3090 or 1× A100 80GB with room to spare)
...
- **vLLM serving MoE:** `8.mlops/10_serving_optimization_end_to_end.md`

## Key Takeaway

MoE = N expert FFN layers + router that sends each token to top-K experts. Mixtral 8×7B: 46.7B total, 12.9B active — 13B compute cost, ~35B quality. Key challenge: load balancing (auxiliary loss forces equal token distribution). Key tradeoff: compute-efficient but memory-hungry (all N experts must be loaded). GPT-4 reportedly uses MoE (~1.8T total, 16 experts/layer, 2 active). Specialization emerges naturally — code tokens go to code experts, financial tokens to finance experts.
