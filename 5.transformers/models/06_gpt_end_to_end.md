...
Same sentence: "cat sat on mat". Same embeddings.
New: causal mask, next-token prediction on ALL positions, autoregressive generation, sampling strategies.
...
BERT                              GPT
─────────────────────────────────────────────────────────────────
Bidirectional attention (↔)       Causal attention (→ only)
[CLS] token prepended             No special tokens
Token + Segment + PE              Token + PE only
MLM: predict 15% masked tokens    LM: predict EVERY next token
Loss on 15% of tokens             Loss on ALL tokens
Best for: understanding tasks     Best for: generation tasks
Fine-tune: classification head    Use as-is or fine-tune (SFT)
Example: BERT-base 110M           Example: GPT-2 117M, GPT-3 175B
─────────────────────────────────────────────────────────────────

THE KEY ARCHITECTURAL DIFFERENCE:
BERT: scores[i,j] = raw score → ALL tokens in context
GPT:  scores[i,j] = raw score for j ≤ i,  -∞ for j > i
      → after softmax: 0 weight on future tokens
...
3. Training signal: both embedding and LM head updated simultaneously
...
### Step 2: Raw Score Matrix S = Q @ Kᵀ / √2
...
Masked score matrix:
         k_cat   k_sat   k_on    k_mat
q_cat:   1.146    -∞      -∞      -∞       ← cat sees only itself
q_sat:   0.896   0.723    -∞      -∞       ← sat sees cat + itself
q_on:    0.405   0.344   0.212    -∞       ← on sees cat+sat+itself
...
```
softmax([0.405, 0.344, 0.212, -∞]):
exp(0.405)=1.500, exp(0.344)=1.411, exp(0.212)=1.236   sum=4.147
...
Mat attends: cat=0.239, sat=0.245, on=0.256, itself=0.260
(same as transformer file — last position has no future tokens to mask anyway)
```
...
c_sat:
   ...
   dim1: 0.543×1.250 + 0.457×0.796 = 0.679 + 0.364 = 1.043
   c_sat = [1.175, 1.043]

c_on:
   dim0: 0.362×1.250 + 0.340×1.085 + 0.298×0.712
       = 0.453 + 0.369 + 0.212 = 1.034
   dim1: 0.362×1.250 + 0.340×0.796 + 0.298×(-0.019)
       = 0.453 + 0.271 - 0.006 = 0.718
   c_on = [1.034, 0.718]
...
**FFN:**
```
cat  (LN=[-1,1]): pre_act=[-0.1,-0.1,0.1,0.0] → h=[0,0,0.1,0] → ffn=[0.030,0.020]
sat  (LN=[1,-1]): pre_act=[0.1,0.1,-0.1,0.0]  → h=[0.1,0.1,0,0] → ffn=[0.070,0.070]
...
```
x_final_cat = [2.250, 2.750] + [0.030, 0.020] = [2.280, 2.770]
...
**W_lm = token_embed.T = [[1.0, 0.2, 0.1, 0.2], [0.5, 0.3, 0.1, 0.4]]**
...
probs: exp→[39.05,3.621,1.657,4.779] sum=49.11
P = [0.796, 0.074, 0.034, 0.097]
Target: sat(1), P(sat)=0.074
```

**Logits at position 1 (sat → predict on):**
```
logits = x_final_sat @ W_lm = [2.286, 1.953] @ W_lm:
   cat: 2.286×1.0 + 1.953×0.5 = 2.286+0.977 = 3.263
   sat: 2.286×0.2 + 1.953×0.3 = 0.457+0.586 = 1.043
   on:  2.286×0.1 + 1.953×0.1 = 0.229+0.195 = 0.424
   mat: 2.286×0.2 + 1.953×0.4 = 0.457+0.781 = 1.238

probs: exp→[26.10,2.837,1.528,3.449] sum=33.91
P = [0.770, 0.084, 0.045, 0.102]
Target: on(2), P(on)=0.045
```

**Logits at position 2 (on → predict mat) — main focus:**
```
logits = x_final_on @ W_lm = [2.113, 0.472] @ W_lm:
...
probs: exp→[10.47,1.759,1.294,1.844] sum=15.37
P = [0.681, 0.114, 0.084, 0.120]
Target: mat(3), P(mat)=0.120
```
...
──────────────────────────────────────────────────────────────────
0 (cat)   [2.280,2.770]   cat(0.796)       sat      0.074
1 (sat)   [2.286,1.953]   cat(0.770)       on       0.045
2 (on)    [2.113,0.472]   cat(0.681)       mat      0.120
──────────────────────────────────────────────────────────────────
Why does "cat" always win? Its embedding [1.0,0.5] has the largest
magnitude. The final hidden states have large positive dim0 values
→ dot product with cat's embedding is always highest.
This is a cold-start problem — model hasn't been trained yet.
After training: x_final_on should produce high logit for "mat".
```

---

## 5. Loss — Computed at ALL Positions
...
**Why GPT's loss > BERT's loss at initialization:**
```
BERT MLM loss: 1.347 (slightly above random 1.386 for 4 words, close)
GPT LM loss:   2.608 (well above random)

Because:
- GPT loss averages over ALL positions (including position 0 which has no context)
- Position 0 must predict sat with ONLY its own embedding — near impossible
...
## 6. Backward Pass (Focus on Position 2: on → mat)
...
```
∂L₂/∂W_lm = x_final_onᵀ @ ∂L₂/∂logits (outer product)
...
After update, x_final_on·mat_embed increases → P(mat|on context) increases.
```
...
```
∂L/∂W2 = h_onᵀ @ ∂L/∂ffn_out = [[0.1],[0.1],[0],[0]] @ [[0.536,0.031]]
        = [[0.054, 0.003],[0.054, 0.003],[0,0],[0,0]]

∂L/∂pre_act_on = ∂L/∂ffn_out @ W2ᵀ ⊙ [1,1,0,0] (ReLU mask)
∂L/∂W1 = LN₁_onᵀ @ ∂L/∂pre_act_on   (LN₁_on = [1,-1])
```
...
Key: v_mat gets NO gradient from position 2's loss!
     ...
     This is correct: mat's value couldn't have affected the on→mat prediction.
```

**∂L/∂Wv = X_inputᵀ @ ∂L/∂V:**
```
Rows 0,1,2 of ∂L/∂V are non-zero.
Row 3 (mat) is zero.
...
New probs: exp→[7.612, 1.745, 1.279, 2.787] sum=13.423
P(cat)=0.567, P(mat)=0.208   (mat: 0.120→0.208 ✓, cat: 0.681→0.567)

L' = -log(0.208) = 1.570   < 2.120 ✓

Loss at position 2 dropped from 2.120 → 1.570 after one step.
P(mat) nearly doubled: 0.120 → 0.208.
```

---

## 9. Sampling Strategies for Generation
...
When to use: when you need reproducible output, or beam search is too expensive.
```

### 9.2 Temperature Sampling
...
exp: [109.6, 3.096, 1.675, 3.400]   sum=117.77
...
exp: [3.238, 1.327, 1.138, 1.358]   sum=7.061
...
T=5.0:  [0.286, 0.244, 0.232, 0.238]  ← nearly uniform
...
k=2: keep cat(0.681) and mat(0.120), zero out sat and on.
   Renormalized: P(cat)=0.681/(0.681+0.120)=0.850, P(mat)=0.150
   Sample from {cat:0.850, mat:0.150}

k=3: keep cat(0.681), mat(0.120), sat(0.114)
   sum=0.915
   Renormalized: P(cat)=0.744, P(mat)=0.131, P(sat)=0.125

k=4 (all): original distribution unchanged.

Effect: eliminates low-probability "tail" tokens that could derail generation.
Fixed k means: at high-certainty positions, even k=2 gives only 2 choices.
...
### 9.5 Combined: Temperature + Top-p (Best Practice)
...
Step 1: top-2 from position 0 logits:
   Beam A: "cat cat"   score=log(0.796)=-0.228
...
## 10. Autoregressive Generation Step by Step
...
Step 1 — Input: [cat, sat], predict next:
   ...
   logits = [3.263, 1.043, 0.424, 1.238]
...
GRADIENT SIGNAL COMPARISON:
   ...
   GPT step:  512 gradient sources (all tokens)
              Token 0 has no context, token 511 has 510-token context
...
INPUT (no [CLS], no segment):
   ...
   PE(0)  PE(1)  PE(2)  PE(3)
    │      │      │      │
[1.0,1.5][1.04,0.84][1.009,-0.316][0.341,-0.590]
    │      │      │      │
    └──────┴──CAUSAL MASK──┘
         Q @ Kᵀ / √2
    ┌────────────────────────────────┐
    ...
    pos0: P(sat)=0.074,  L₀=2.604
    pos1: P(on) =0.045,  L₁=3.101
    pos2: P(mat)=0.120,  L₂=2.120
    L_total = 2.608
...
BACKWARD (position 2):
   ...
   After update: P(mat) = 0.208 ✓   L' = 1.570 ✓
```
...
│    Top-p=0.9:   {cat:0.744, mat:0.131, sat:0.125} (adaptive)       │
│  After update: L'=1.570 at pos 2 (was 2.120) ✓                     │
└─────────────────────────────────────────────────────────────────────┘
```
...
# [0.362, 0.340, 0.298, 0.000]  ← mat is 0 due to causal mask ✓
...
X_final = X_attn + H @ W2 + b2  # (4, 2)

# LM Head (weight tying)
logits = X_final @ W_lm   # (4, 4)  each row = logits over vocab
probs  = np.exp(logits - logits.max(-1, keepdims=True))
probs /= probs.sum(-1, keepdims=True)

print("Logits at pos 2 (on):", np.round(logits[2], 3))
# [2.349, 0.565, 0.258, 0.612]
print("Probs at pos 2:", np.round(probs[2], 3))
...
print(f"Greedy:     {words[sample_next_token(context_logits, 'greedy')]}")
print(f"T=0.5:      {words[sample_next_token(context_logits, 'sample', temperature=0.5)]}")
print(f"T=2.0:      {words[sample_next_token(context_logits, 'sample', temperature=2.0)]}")
print(f"Top-k=2:    {words[sample_next_token(context_logits, 'sample', top_k=2)]}")
print(f"Top-p=0.9:  {words[sample_next_token(context_logits, 'sample', top_p=0.9)]}")

# ─── Backward + Update ───────────────────────────────────────────────────
# Focus on position 2 (on → mat)
one_hot = np.zeros(4); one_hot[targets[2]] = 1.0
dL_dlogits = probs[2] - one_hot                  # [0.681, 0.114, 0.084, -0.880]
dL_dW_lm   = X_final[2:3].T @ dL_dlogits.reshape(1,-1)  # (2,4)
...
p2 = np.exp(logits2 - logits2.max()); p2 /= p2.sum()
loss2 = -np.log(p2[targets[2]])
print(f"\nAfter update pos 2: P(mat)={p2[3]:.3f}, L'={loss2:.3f}")
# P(mat): 0.120 → ~0.208, L': 2.120 → ~1.570 ✓
```
...
W1 = torch.tensor([[0.5,0.3,0.2,0.1],[0.4,0.2,0.3,0.1]], requires_grad=True, dtype=torch.float32)
b1 = torch.zeros(4, requires_grad=True)
W2 = torch.tensor([[0.5,0.3],[0.2,0.4],[0.3,0.2],[0.1,0.5]], requires_grad=True, dtype=torch.float32)
...
# LM logits (weight tying: W_lm is (4,2), we need (2,4) for projection)
logits = X_final @ W_lm.T   # (4,4)

# Cross-entropy loss on positions 0,1,2 (predicting positions 1,2,3)
loss = F.cross_entropy(logits[:3], targets)
print(f"Loss: {loss.item():.3f}")   # 2.608

# ─── Backward ────────────────────────────────────────────────────────────
loss.backward()
print("dL/dW_lm:", W_lm.grad.round(decimals=3))
...
def generate(model_fn, start_ids, max_new_tokens=4, temperature=1.0, top_p=0.9):
    ...
    for _ in range(max_new_tokens):
        ...
        X = token_embeds[inp] + make_pe(len(ids), 2)

        Q = X @ Wq; K = X @ Wk; V = X @ Wv
        n = Q.size(0)
        mask = torch.triu(torch.ones(n,n), diagonal=1).bool()
        s = (Q @ K.T / (Q.size(-1)**0.5)).masked_fill(mask, float('-inf'))
        A = F.softmax(s, dim=-1)
        C = A @ V
        X_attn = X + C
        X_ln = F.layer_norm(X_attn, [2], gamma, beta)
        ...
        p_filtered[sorted_idx] = sorted_p
...
# e.g., "cat sat on mat" (at random init, expect 'cat' heavy output)
```
...
print("T=0.5:", tokenizer.decode(t05_ids[0]))
print("T=2.0:", tokenizer.decode(t20_ids[0]))

# ─── Top-k Sampling ──────────────────────────────────────────────────────
with torch.no_grad():
    topk_ids = model.generate(**inputs, max_new_tokens=10,
                               do_sample=True, top_k=50, temperature=0.8)
print("Top-k=50:", tokenizer.decode(topk_ids[0]))

# ─── Nucleus (Top-p) Sampling ────────────────────────────────────────────
with torch.no_grad():
    nucleus_ids = model.generate(**inputs, max_new_tokens=10,
                                  do_sample=True, top_p=0.9, temperature=0.8)
print("Top-p=0.9:", tokenizer.decode(nucleus_ids[0]))
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Greedy at T=1 produces         argmax misses likely        Use T=0.8+top_p=0.9
repetitive output               alternatives                for balanced sampling

Setting T=0 exactly            logits/0 = inf → NaN        Use T=1e-8 instead,
                               → all probabilities NaN      or just use greedy

Not shifting labels             labels = input_ids (no shift)  labels = input_ids[:,1:]
in causal LM loss              → predicting current token      or use model built-in
                               (trivial task, loss≈0 but wrong) shift

Top-p with floating point      cumsum never exactly =p      Use ≥p threshold,
precision                       → keeps too many/few         not ==p

Temperature applied BEFORE     Should apply BEFORE top-k/p  Always: T → topk/p → softmax
top-k/p filtering               otherwise scale changes      (in that order)
relative probabilities

Not disabling dropout           Dropout on at inference       model.eval() before generate
at inference                    → nondeterministic            torch.no_grad()

Padding on wrong side           GPT needs LEFT padding        tokenizer.padding_side='left'
                                for batch generation          before batch tokenization

KV cache not cleared            Cache from previous prompt    model(use_cache=True) for
between unrelated prompts       leaks into new generation     streaming; clear between
                                → contaminated output         independent generations

Weight tying requires           If W_lm = embed.T (tied),    freeze=False for both
tied params both updating       updating only one breaks      or use nn.Parameter shared
                                the tied relationship          reference
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```
...
```
A: The causal (look-ahead) mask sets attention scores to -∞ for all future
positions (j > i) before softmax, resulting in 0 attention weight on future tokens.

It's necessary because GPT is trained autoregressively:
   Training: given [cat, sat, on, mat], predict [sat, on, mat, ?] simultaneously
   The mask ensures: when predicting "on" (position 2), the model can't see "mat"
   This prevents "cheating" — without the mask, predicting "on" is trivial
   (it's already in the input at position 3)

At inference (generation), we generate tokens one at a time. The causal mask
ensures training and inference use the same attention pattern — no mismatch
(unlike BERT's train:[MASK] vs inference:real token mismatch).
```

**Q: What is weight tying and why is it used in GPT?**
```
A: Weight tying sets the output projection W_lm = token_embedding.T (transposed).
The same matrix serves two roles: input lookup (embedding) and output scoring.

Benefits:
1. Saves parameters: vocab × d (50K × 768 = 38M fewer params for GPT-2)
2. Consistency: predicting token w means "does x_final point toward w's embedding?"
   The dot product x_final · embed[w] = cosine-like similarity
3. Training efficiency: gradients flow to embeddings from BOTH input and output,
   improving embedding quality

Gradient with weight tying:
   embed[w] receives gradient from: (a) input embedding lookup + (b) output logit
   Both pull the embedding toward representations useful for prediction.
```

**Q: Compare greedy, temperature, top-k, and top-p sampling. When to use each?**
```
A: 
Greedy: always picks argmax. Deterministic, fast. Good for factual QA where
        you want the single best answer. Problem: repetitive loops.

Temperature (T): divide logits by T before softmax.
   T<1: sharper (more confident) → good for factual, structured output
   T>1: flatter (more diverse) → good for creative writing
   T→0: equivalent to greedy; T→∞: uniform random

Top-k: keep only the k highest tokens. Removes long tail.
   Problem: k is fixed regardless of distribution certainty.
   At high certainty (P=[0.95,0.04,...]) k=50 includes garbage.
   At high uncertainty (P=[0.02,0.02,...]) k=50 misses too many options.

Top-p (nucleus): keep smallest set with cumulative probability ≥ p.
   Adapts to certainty: certain → small set, uncertain → larger set.
   p=0.9 is the most common choice. Better than top-k in practice.

Best practice: Temperature (T=0.7-1.0) + top-p (p=0.9). This gives
diversity (temperature) while preventing degenerate choices (top-p).
```

**Q: Why does GPT compute loss on all tokens while BERT only uses 15%?**
```
A: Different pretraining objectives:

BERT (MLM): randomly mask 15%, train to predict only those 15%.
   The other 85% are still in the input as context.
   Loss = cross-entropy averaged over masked positions only.
   Pro: each masked token uses full bidirectional context → rich signal.
   Con: 85% of tokens contribute NO gradient per step → less efficient.

GPT (CLM): predict every next token.
   Loss = cross-entropy averaged over ALL n-1 positions.
   Pro: 100% of tokens produce gradient → more signal per example.
   Con: early tokens have very little context (token 0 sees only itself).

Practical consequence:
   BERT learns from ~77 gradient sources per 512-token sequence
   GPT learns from ~511 gradient sources per 512-token sequence
   BUT GPT's early positions have poor context → noisier gradients.
   Both converge: BERT typically needs more epochs, GPT needs more data.
```

**Q: What is perplexity and how does it relate to cross-entropy loss?**
```
A: Perplexity = exp(average cross-entropy loss per token)
             = exp(-1/N × Σ log P(xₜ|x₁...xₜ₋₁))

Interpretation: "on average, how many equally likely choices does the model
               think there are at each token?"
   Perplexity=10: model is as confused as choosing uniformly from 10 options
   Perplexity=2: very confident, like a coin flip

In our walkthrough:
   L = 2.608 → PPL = exp(2.608) = 13.6
   The model acts like it has ~14 equally likely choices at each position.
   
   Random model over 4-word vocab: PPL = 4.
   Our untrained model: PPL = 13.6 → worse than random on this sequence
   (because word-magnitude bias pushes everything toward "cat").

Good models: GPT-2 on web text ≈ PPL 25-35, GPT-3 ≈ 20.
```
...
- **Encoder-Decoder (models/03):** T5/BART combine causal decoder with bidirectional encoder

## Key Takeaway
GPT = transformer decoder with causal mask + next-token prediction on ALL positions. The causal mask is the only architectural difference from BERT. Weight tying connects input embeddings to output logits. Loss at every position (not just 15%) means more gradient but noisier early signals. Generation is autoregressive: one token at a time, reusing past K/V (KV cache). Sampling strategies — temperature, top-k, top-p — control the creativity/coherence tradeoff. Use top-p=0.9 + temperature=0.8 as the default starting point.
