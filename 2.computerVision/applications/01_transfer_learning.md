...
```
ImageNet-trained CNN feature hierarchy:
...
Key insight: Early features are universal. Reuse them. Only relearn task-specific top layers.
```
...
### What It Means
Remove the classification head (final FC layer). Freeze all backbone weights. Add new head. Train only the new head.
...
```python
def build_fine_tuned(backbone_name='resnet50', num_classes=10,
...
model = build_full_finetune(num_classes=5)
...
| Large (>50K) | Similar | Full fine-tune or train from scratch | 1e-4 uniform |
...
```python
# Simple domain adaptation: fine-tune with target data
# (even unlabeled — use pseudo-labels or self-supervised objectives)

# 1. Self-supervised pre-training on target domain (no labels needed)
# 2. Then fine-tune supervised on labeled target

# SimCLR-style pretraining on target domain
...
**Forgetting ImageNet normalization.**
If using ImageNet-pretrained weights, always normalize with ImageNet stats (mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]). Using wrong normalization → backbone receives unexpected inputs → bad features → poor accuracy.

**Catastrophic forgetting on small LR but too many epochs.**
Even with LR=1e-4, training for 100 epochs on a small dataset overwrites pretrained features. Use early stopping or cosine annealing with warmup.

**BatchNorm running stats during freezing.**
When backbone is frozen, BatchNorm layers in frozen blocks should be set to eval mode — they use running stats, not batch stats. Otherwise BN tracks stats for the frozen layers during training and running stats drift.

```python
# Fix: set frozen BN layers to eval mode
...
**Not using the right pretrained weights.**
`models.resnet50(pretrained=True)` uses IMAGENET1K_V1 (old). `weights='IMAGENET1K_V2'` uses improved weights (+2-3% accuracy). Always specify the weights version explicitly.

**Unfreezing too fast (catastrophic forgetting).**
Going from feature extraction directly to full fine-tune with LR=1e-3 → wipes pretrained weights. Always use discriminative LRs and reduce LR for backbone layers.
...
model.fc = nn.Sequential(nn.Dropout(0.5), nn.Linear(2048, len(train_ds.classes)))
...
|-----------|----------|-----|
| Backbone architectures | `../fundamentals/02_cnn_architectures.md` | Which backbone to choose |
| Data augmentation basics | `../fundamentals/01_cnn_mechanics.md` | Augmentation for small datasets |
| Focal loss for imbalanced | `../../1.deep learning/fundamentals/06_specialized_losses.md` | Fine-tuning with class imbalance |
| Feature extraction → sklearn | `../../0.machine learning/algorithms/02_tree_models.md` | Use extracted features with XGBoost |
| Domain adaptation in NLP | `../../3.nlp/` (future) | BERT fine-tuning = same concept |
...
**Your domain (document automation):** ImageNet→document is a big domain shift. Best strategy: ResNet-50 feature extraction → evaluate → partial fine-tune layer4+head → full fine-tune if dataset > 5K images. Heavy augmentation: perspective transforms, blur, JPEG compression artifacts (simulate real scan conditions).
