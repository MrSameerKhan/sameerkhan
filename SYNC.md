...
| **What I did** | **(A) Read all 11 files of `8.agents/` and fixed 6 real defects** — 3 broken imports in `01b` (two pointed at `langchain` not `langgraph`; one was a literal `import import` SyntaxError); `sanitize_tool_output(str, ...)` in `01b` shadowed the builtin and read an undefined `output`; the FM1 rescue regex in `02` read `\{s*"name"` (zero-or-more literal *s*) so it matched compact JSON and **silently missed pretty-printed JSON**, which is most of what a model emits; a duplicated nested `for chunk in graph.stream(...)` in `04`; a ReAct answer in `01b` that attributed hallucination to ReAct instead of CoT and then listed that flaw as a ReAct *benefit*; and two consecutive sentences in `03` both opening "LangGraph is the right tool for" with opposite jobs, plus a garbled Key Takeaway. **Correction to an earlier claim of mine:** `claude-opus-4-6` / `claude-sonnet-4-6` ARE real (previous-generation) IDs and `01b`'s costed dry-run prices Sonnet 4.6 correctly at $3/$15 — no error there, only a generation gap against `00`, left alone. **(B) Built `code_practice/12_agents_from_scratch/` out to all 37 modules.** Restructured into 12 block folders A–L (shared `_providers.py` / `_tools.py` / `_fake_model.py` stay at the phase root because notebooks cannot be imported; every module opens `sys.path.insert(0, "..")`). Modules 02–37 are `.ipynb`; 01 stays `.py`. Every module now paired with a `_details.md` per CLAUDE.md. **12 of 37 verified end-to-end** — all the ones needing no API key, plus 01 and 02 live. **(C) Three modules failed their own claims on first run and were fixed:** module 13's guard 2 claimed to catch FM2 and returned the prose plan as an answer, identical to the *unguarded* loop; module 20 "forked" a decision point and silently got back the **recorded** answer (re-resuming an answered interrupt replays it — a fresh `thread_id` is what branches); module 21 described the reducer collision as silent last-write-wins, but **LangGraph 1.2.1 raises `InvalidUpdateError`** — which makes `04_langgraph_deep.md` §13 stale. Module 02's own lesson said "OUTPUT stays flat"; measured output was 51/16/66, so it now reads "OUTPUT does not grow". **(D) Env:** keys must live in `~/.zshenv`, not `~/.zshrc` — an interactive-only file is never read by a notebook kernel. Ollama: `llama3.2` (3B) pulled for work, `llama3.2:1b` **kept deliberately** as the failure instrument for modules 07/12; `llama2` removed. Corrected this file's claim that the Mac is 16 GB — it is **8 GB**. |
| **Files changed** | FIXED: `8.agents/01b_agents_end_to_end.md`, `02_agent_reliability_patterns.md`, `03_langchain_primer.md`, `04_langgraph_deep.md`. NEW: 36 notebooks + 37 `_details.md` across `code_practice/12_agents_from_scratch/{A_the_wire … L_evaluation}/`. REWRITTEN: that phase's `README.md` (block folders, notebook convention, Built-vs-Run-live status, evidence table). UPDATED: `.gitignore` (generated traces, `__pycache__`, `_stdio_server.py`), `SYNC.md`. |
| **Ladder run progress (13 Sep)** | Blocks **A, B, C run**. Run in-notebook with outputs saved: **01–04, 06, 08–10**. Executed offline by agent (no key needed, notebooks carry no saved outputs): **12, 13, 14, 20, 21, 24, 25, 34, 36, 37**. **Never run anywhere: 15–19, 22, 23, 26–33, 35.** **Needs a re-run after a code fix: 05, 07, 11.** |
| **Defects found by RUNNING (not reading)** | **05** — `classify()` discarded `r.usage`, so a module about cost accounting never charged for its own router; also its predicted 70–90% saving was wrong, measured **18%**, because one of three questions genuinely needed Opus and dominated the bill. **07** — the loop reported `ok=True` on an answer computed from nothing: the model emitted an `Action` *and* a `Final Answer` in one turn, `FINAL_RE` was checked first, and the returned answer was the literal `[calculated amount]`. **11** — asserted the ~10x agent multiplier while printing **0.7x**; input was 10.2x exactly as claimed, but output was 0.4x and output is priced ~4x, so a verbose single call out-cost a terse 2-turn agent. **06** — the refine loop's output carried conversational wrapper text that a chained step would parse as content. All four were modules asserting something their own output contradicted — same class as 13, 20, 21 found earlier. |
| **⚠️ REPO MOVED OFF iCLOUD (13 Sep)** | **Root cause of VS Code sync spinning 10+ min:** the repo lived at `~/Desktop/sameerkhan`, and iCloud "Desktop & Documents" sync was ON with **iCloud storage FULL (50/50 GB, "Saving Is Paused")** and local disk at ~7.5 GB free. iCloud evicted **1,388 repo files** to placeholders ("dataless"); VS Code's `git status -z -uall` blocked reading them and **hung with zero CPU — six processes, some for 40 hours**. Plain `git status --untracked-files=no` was unaffected. **Fix:** pushed everything, turned iCloud Desktop & Documents sync OFF, fresh-cloned to **`~/code/sameerkhan`**. **Never keep a git repo inside an iCloud-synced folder.** On this Mac: work in `~/code/`, not `~/Desktop/`. Re-runs of modules 05, 06, 07 are included in this push; their `_details.md` still describe the pre-fix runs and need updating from the new outputs. |
| **Known-stale after this session** | `8.agents/04_langgraph_deep.md` §13 says the parallel-write collision is silent; on LangGraph 1.2.1 it **raises**. `STRUCTURE.md` lists no `code_practice` phase individually and its appendix is dated 14 May — phase 12 is absent, but so is every phase. |
| **Could NOT verify** | The ladder README's claim of a **2026-07-28 MCP revision** making the protocol stateless and introducing **MRTR** (deprecating `sampling` / `elicitation` / `roots`). No primary source reachable; knowledge cutoff is earlier. Modules 32–35 are therefore built **only** on JSON-RPC 2.0 and the stable tools / resources / prompts core, and module 34 was renamed from `34_mcp_transports_and_mrtr` → `34_mcp_transports`. **Confirm against the spec before adding MRTR content.** |
...
| **Ollama / local Llama** | ✅ **modestly better than Windows.** Unified memory beats 4 GB VRAM, but this M1 has **8 GB, not 16** — a 3B at Q4 (~2 GB) is comfortable; an 8B (~4.7 GB) is not, alongside VS Code. Both `llama3.2` (3B, the working model) and `llama3.2:1b` (kept as the failure instrument for modules 07/12) are pulled. Only 2 files touch it: `_providers.py` (`"local"` entry) and `A_the_wire/01_standard_llm_call.py` Cell 4. Model name is a one-line change. | limited to ~3B |
...
| 11 | Interview drills | — | n/a | Plans + QA banks, not runnable sessions |
| **12** | **Agents from scratch** | **M01-M37** | **✅ 12 of 37** | 12 block folders A–L. Verified = every module needing no API key, plus 01 and 02 live. `.ipynb` from module 02 on; shared `_*.py` at the phase root. Mac: keys in `~/.zshenv`, `llama3.2` + `llama3.2:1b` both kept. |
