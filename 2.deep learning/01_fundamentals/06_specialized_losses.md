...
Behavior: Well-classified example: p_t = 0.95 → (1-0.95)² = 0.0025 → contribution ~0
...
**Problem it solves:** Sequence-to-sequence where: · Input and output have different lengths (input: image columns or audio frames, output: characters) · No frame-level alignment labels (you don't know which frame maps to which character) · Classic in OCR, ASR, handwriting recognition.
...
```
CTC loss = -log Ptarget(input) = -log Σ P(alignment) over all valid alignments
...
# targets: [N, S] or ID concatenated targets
...
**Limitation:** Dead negatives: once d > margin, negative pairs contribute zero gradient. Needs hard negative mining.
...
**Hard negatives:** negatives that are close to anchor in embedding space (hardest to separate). **Semi-hard:** negatives farther than positive but within margin.
...
where:
  ...
  Denominator = all other 2N-1 samples in batch (negatives)

Key insight: Larger batch size = more negatives = better representations.
SimCLR typically uses batch size 4096-8192.
```

```python
def nt_xent_loss(z1, z2: [batch_size, embedding_dim], L2 normalized):
...
```
Dice = 2 · |A ∩ B| / (|A| + |B|)
     = 2 · Σ(p_i · q_i) / (Σp_i + Σq_i)

Dice Loss = 1 - Dice

where:
  p_i = predicted probability for pixel i
  q_i = ground truth (0 or 1) for pixel i
```
...
**CTC: input_lengths must be exact.** If you pad sequences and pass max_length as input_lengths instead of actual lengths — CTC treats padding frames as real → alignment broken → loss explodes or NaN. Always pass true sequence lengths, not padded lengths.

**CTC: blank class must be included in num_classes.** `vocab_size = num_chars + 1` for blank. Forgetting the blank token → wrong output dimension → CUDA error or shape mismatch.

**Triplet: random sampling = dead loss.** ~99% of random triplets are easy (already satisfy margin) → zero gradient → model doesn't train. Always use online hard or semi-hard mining.

**Focal loss: γ tuning matters.** γ=0 is standard CE. γ=2 is RetinaNet default. Higher γ → more focus on hardest examples — can hurt if your hard examples are actually mislabeled. Tune γ on validation set.

**NT-Xent: small batch → bad negatives.** SimCLR needs batch 4096+ for meaningful contrastive signal. On small batch (256), most negatives are too easy → representations don't learn well. Use MoCo queue (momentum encoder + queue of negatives) for memory-efficient contrastive learning.

**Label smoothing + distillation interaction.** If teacher was trained with label smoothing and student also uses it → double-smoothing → too much uncertainty. Use one or the other.
...
class CTCOCRModel(nn.Module):
    ...
    def __init__(self, num_classes):
        ...
        # BiLSTM: sequence modeling over CNN features
        self.rnn = nn.LSTM(128, hidden_size=8, num_layers=2,
                           bidirectional=True, batch_first=False)
        # Projection: hidden → vocabulary (including blank)
        self.head = nn.Linear(hidden_size * 2, num_classes + 1)  # +1 for blank

    def forward(self, x):
        x = F.[8, 1, H, W]  # (grayscale text line image)
        features = self.cnn(x)                 # [B, C, H/4, W/4]
...
|-----------|----------|-----|
| CTC Loss | `../02_architectures/03_rnn_lstm_gru.md` | BiLSTM is the typical CTC encoder |
| Contrastive / NT-Xent | `../02_architectures/04_transformer.md` | BERT pre-training uses MLM, not contrastive — contrast with SimCLR |
| Contrastive training (text embeddings) | `../../4.nlp/02_embeddings/06_contrastive_training.md` | Hard negatives, in-batch negatives, supervised contrastive |
| Focal Loss | `../../3.computerVision/02_applications/` | RetinaNet, YOLO — detection loss baseline |
| Dice Loss / segmentation | `../../3.computerVision/02_applications/` | UNet / DeepLab training for document layout |
| Label Smoothing | `01_foundations.md` | Cross-entropy base, calibration concept |
| Diffusion / Flow Matching loss | `../02_architectures/05_generative.md` | DDPM, score matching, rectified flow |
| Distillation (KL loss) | `../../8.agents/02_agent_reliability_patterns.md` | Student-teacher training, KD = soft-CE |
...
**For your domain (document automation): CTC is the most critical loss to know cold: blank token, collapse rule, why input_length ≥ target_length, how to combine with attention decoder.**
