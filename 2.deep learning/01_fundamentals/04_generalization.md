...
**Bias** = error from wrong model assumptions (model too simple). **Variance** = sensitivity to training data fluctuations (model too complex).
...
During backprop: ∂L_total/∂W = ∂L_data/∂W + λW
```
...
Epoch 7: val_loss = 0.45  > 0.45 + patience=2, stop here
...
### Modern Mixing Augmentations (MixUp / CutMix / CutOut)

Beyond per-sample augmentations, **mixing augmentations** combine pairs of training samples — a powerful regularizer with very low compute cost.

| Method | Idea | Loss |
|--------|------|------|
| CutOut | Mask a random rectangle in the input | Normal CE on unmasked label |
| MixUp | x̂ = λ·x_a + (1-λ)·x_b with λ ~ Beta(α, α) | λ·CE(ŷ, y_a) + (1-λ)·CE(ŷ, y_b) |
| CutMix | Paste a random patch of x_b into x_a; λ = patch area fraction | Same mixed-label loss as MixUp |
| MixUp + Label Smoothing | Combine with label smoothing | Strong baseline on ViT, ConvNeXT training recipes |
| AugMix / RandAugment / TrivialAugment | Compose / randomize basic augmentations | Standard in modern vision training recipes |
...
**Senior interview answer:** "MixUp + CutMix + RandAugment is the standard ViT/ConvNeXt training recipe today. They reduce overfitting, improve calibration, and provide implicit ensembling without the inference cost. Beyond classification, MixUp variants help in NLP (Wangchanberta) and audio."
...
Large batches (>512): deterministic gradients → converge to **sharp minima** → worse generalization (Keskar et al. 2017). Small batches: noisy gradients → converge to **flat minima** → better generalization.
...
**Multi-class metrics:** · **Macro avg:** each class weighted equally — use when all classes equally important · **Weighted avg:** weighted by class support — use when class distribution reflects real world
...
**Parameters:** learned by gradient descent (W, b). **Hyperparameters:** set before training (LR, batch size, dropout rate, number of layers).
...
**1. Accuracy on imbalanced data is meaningless.** A model predicting all "healthy" on 990/10 cancer data gets 99% accuracy and detects zero cancer. Always check class distribution before choosing a metric.

**2. Data leakage is silent — it always inflates your results.** Preprocessing before splitting is the most common form. Normalizing on the full dataset, computing category statistics on full dataset — these are all leakage. Always split first, then fit any statistics on train only.

**3. Large batch size harms generalization.** Not a hardware problem — a fundamentals one. Large batches compute near-exact gradients — converge to sharp minima that overfit. Small batches inject useful noise. If you scale batch size for speed, also scale LR and add warmup.

**4. Test set touched more than once = invalidated.** Each time you look at test results and change something, the test set has leaked. In published research, this causes benchmark overfitting at the community level. In production, this causes overconfident models.

**5. val_loss improving ≠ generalization improving.** Validation metrics can plateau or even slightly worsen while training loss improves significantly. Monitor the gap (overfit ratio = val_loss/train_loss), not just val_loss alone.
...
Large batches compute near-exact gradients (low noise) — gradient descent converges toward sharp minima. Sharp minima are regions where a small perturbation to weights causes large increases in loss — the model overfits to the training distribution. Small batches inject gradient descent noise → more → finds flat minima that generalize across the distribution. This is not just a theory — Keskar et al. 2017 showed this empirically on multiple benchmarks.
