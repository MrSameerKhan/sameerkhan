...
Input [784] → Hidden1 [256] → Hidden2 [128] → Output [10]
               ↓                 ↓
             a1 = ReLU(W1·x+b1)  a2 = ReLU(W2·a1+b2)  ŷ = softmax(W3·a2+b3)
```
...
```python
a1 = W1 · x + b1    # linear transform
h1 = ReLU(a1)        # activation
a2 = W2 · h1 + b2
h2 = ReLU(a2)
a3 = W3 · h2 + b3
ŷ  = softmax(a3)     # output layer
```
...
## The FFN in Transformers Is an MLP

The dominant use of MLPs today is **inside transformer blocks** — the "feed-forward network" (FFN) sandwiched between attention layers is a 2-layer MLP. In modern LLMs (LLaMA, Mistral, Gemma), this FFN is replaced by **gated MLPs:**

```python
# Standard FFN (BERT, GPT-2):  Linear(d → 4d) → GELU → Linear(4d → d)
# SwiGLU FFN (LLaMA, Mistral): [Swish(W_gate) ⊙ W_down] · W_down
#                               with hidden_dim = 2.67d to keep param count similar
```

The gating mechanism (element-wise product with a learned gate) reliably outperforms the plain MLP at the same parameter count — one of the few changes since the original transformer that stuck. The FFN accounts for ~⅔ of compute in a transformer block, so its design matters. See `../01_fundamentals/05_modern_components.md` for SwiGLU details and `02_architectures/09_mixture_of_experts.md` for sparse MoE FFNs (Mistral, DeepSeek-V3).

### MLP-Mixer and Friends (All-MLP Vision)

In 2021 Google proposed **MLP-Mixer** — replace attention in ViT with two MLPs (one mixing tokens, one mixing channels). Surprisingly competitive on ImageNet. Variants: **gMLP** (with gating), **ResMLP**, **ConvMixer**. None has dethroned attention or convolutions in practice, but they're cited as evidence that the macro-architecture (token mixing + channel mixing + residual + norm) matters more than the specific mixer operator.

**Senior interview answer:** "MLP-Mixer is a useful reference because it shows pure MLPs can match ViT on vision when given enough data and modern training recipes. It hasn't replaced ViT or CNNs in production, but it clarified the relative importance of macro-architecture (residual + norm + token/channel mixing) vs the specific mixer operator."

---

## Gotchas

**1. MLPs on images ignore spatial structure.** Flattening a 28×28 image to 784 loses all spatial information. A pixel in the top-left corner has no special relationship to its neighbor — the MLP treats them all equally. This is why CNNs are far more parameter-efficient for images.

**2. Parameter explosion with large inputs.** Input size 1000 → hidden 1000: already 1M parameters in one layer. For 224×224 RGB image (150K inputs) → 1000 neurons: 150M parameters in one layer alone. Completely impractical.

**3. Depth without residuals degrades.** Deep MLPs (>5-6 layers) suffer vanishing gradients without residual connections. Either use shallow MLPs or add skip connections.

**4. Universal approximation ≠ practical.** The theorem says one wide hidden layer is enough — but this requires exponentially many neurons. In practice, depth + regularization + proper architecture beats width.
...
- Builds on: `../01_fundamentals/01_foundations.md` — neuron equation, activations, loss
