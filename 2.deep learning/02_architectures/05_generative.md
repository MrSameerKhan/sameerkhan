...
Standard sampling z ~ N(μ, σ²) is not differentiable.
Fix: z = μ + σ·ε where ε ~ N(0,1) — gradient flows through μ,σ; ε is just noise.

### KL Annealing
Start training with KL weight = 0, gradually increase to 1.
Without annealing → posterior collapse (encoder ignores x, outputs prior N(0,1), decoder learns mean).
...
Replace JS divergence with Wasserstein distance → smoother gradients, better training signal.
Gradient penalty (WGAN-GP): penalize ‖∇D(x̂)‖₂ = 1 constraint (Lipschitz condition).
...
```
Input (noisy image x_t) + timestep t embedding
    ↓
...
**GAN: Mode collapse.**
Generator learns to produce a few high-scoring samples, ignores diversity. D can't distinguish → G keeps repeating. Fix: Wasserstein loss + gradient penalty (WGAN-GP), minibatch discrimination, diverse noise.

**GAN: Training instability.**
D becomes too strong → G gradients vanish (log(1-D(G(z))) → 0). Fix: non-saturating loss, balance D/G update steps, lower D LR.

**VAE: Posterior collapse.**
KL term dominates → encoder outputs prior N(0,1) regardless of input → decoder ignores latent z. Fix: KL annealing (β-VAE), free bits, gradient blocking on KL.

**VAE: Blurry outputs.**
MSE reconstruction loss encourages averaging over modes. Fix: perceptual loss (VGG features), VQ-VAE (discrete latent space, used in DALL-E 1).

**Diffusion: Slow inference is a real cost.**
50 DDIM steps × UNet forward pass = significant latency. In production: use quantized UNet (FP16/INT8), SDXL-Turbo (4 steps). Don't benchmark with T=1000.

**Diffusion: Guidance scale.**
Classifier-free guidance (CFG) scale controls quality vs diversity. Scale=1: unconditional. Scale=7-10: sharp, prompt-following. Scale>15: oversaturated, artifacts. Tune per use case.

**All generative models: Evaluation is hard.**
FID (Fréchet Inception Distance) is the standard but imperfect. Low FID ≠ useful outputs. Supplement with: LPIPS (perceptual similarity), CLIP score (text-image alignment), human eval.
...
z = torch.randn(16, latent_dim)          # sample from prior N(0,1)
...
| Loss functions (ELBO, Wasserstein) | `../fundamentals/01_foundations.md` | BCE/MSE building blocks used in VAE loss |
| BatchNorm in DCGAN | `../fundamentals/03_training_stability.md` | BN in generator; spectral norm in discriminator |
| KL divergence | `../fundamentals/01_foundations.md` | Information-theoretic perspective on loss functions |
...
**VAE** = stable training, interpretable latent space, blurry outputs → anomaly detection, latent feature learning.
**GAN** = sharp outputs, hard to train → largely replaced by diffusion for images, still used in video and domain adaptation.
**Diffusion** = current SOTA for generation, text-conditional via cross-attention, slow inference (mitigated by distillation).

For your domain: diffusion → synthetic document generation + augmentation. VAE → anomaly detection on documents. Know the reparameterization trick and CFG cold — they come up in every generative models interview.
