...
1. Smoothness assumption:
   If x₁ ≈ x₂ (close in input space) → y₁ ≈ y₂ (same label)
...
# Iter 4: added 287 pseudo-labels,  pool=46,964, test acc = 0.873
# Iter 5: no confident predictions → stopping
# Final: +6.1% accuracy using 2,836 pseudo-labeled samples
```
...
Confidence distribution (dry run):
  max(p(x)) > 0.99: 480 samples  (very confident)
...
```
Algorithm:
  1. Build graph: nodes = all data points, edges = similarity w(xᵢ, xⱼ)
     w(xᵢ, xⱼ) = exp(-||xᵢ - xⱼ||² / 2σ²)   ← Gaussian kernel

  2. Construct transition matrix T:
     Tᵢⱼ = w(xᵢ, xⱼ) / Σₖ w(xᵢ, xₖ)          ← row-normalize

  3. Propagate: F ← α·T·F + (1-α)·Y₀
     α = propagation factor (0.9 typical)
     Y₀ = initial label matrix (1-hot for labeled, 0 for unlabeled)
...
Algorithm:
  1. Train classifier f₁ on View 1 using labeled set L
  2. Train classifier f₂ on View 2 using labeled set L
...
Why it works: f₁ and f₂ are independent → each provides new information
...
Example in NLP:
  ...
  → classifier trained on text helps bootstrap structural classifier and vice versa
```
...
def co_training(X1_l, X2_l, y_l, X1_u, X2_u, n_iter=30, pool_size=75, k=10):
    ...
    for _ in range(n_iter):
        ...
        top_k1 = p1.max(axis=1).argsort()[-k:]
        X2_l = np.vstack([X2_l, X2_pool[top_k1]])
        y_l2 = np.append(y_l2, p1[top_k1].argmax(axis=1))

        # clf2 labels most confident → add to clf1's training
        p2 = clf2.predict_proba(X2_pool)
        top_k2 = p2.max(axis=1).argsort()[-k:]
        X1_l = np.vstack([X1_l, X1_pool[top_k2]])
        y_l1 = np.append(y_l1, p2[top_k2].argmax(axis=1))
...
A: When labeling is expensive or slow but unlabeled data is abundant. Common in: medical imaging (radiologist labeling costs $$/hour, but unlabeled scans are cheap), document classification (1000 labeled docs, 100K unlabeled), NLP (few human-labeled examples, large corpus). Also useful when a model trained on labeled data is deployed — production data can be used as unlabeled set.
...
- **Active learning (related concept):** label the most uncertain samples instead of random
- **Data augmentation:** `3.computerVision/`
