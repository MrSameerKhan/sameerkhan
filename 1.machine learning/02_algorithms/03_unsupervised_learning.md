...
Random init → bad clusters if centroids start near same region. K-Means++: first centroid random, each subsequent centroid chosen with probability ∝ distance² from nearest existing centroid → spread out initial centroids → faster convergence, better solution.
...
3. Sensitive to outliers (outliers distort centroids)
...
print(f"Clusters: {n_clusters}, Noise points: {n_noise} ({(labels==-1).mean()*100:.1f}%)")
```
...
3. Large datasets → DBSCAN is O(n²); use HDBSCAN for speed
...
`n_neighbors`: controls local vs global structure balance (15-50 typical). `min_dist`: how tightly clustered points are in embedding (0.0-1.0).
...
**Idea:** Anomalies are easier to isolate — they need fewer random splits to be separated. Build random trees where each split randomly selects a feature and threshold. Anomaly score = average path length to isolate the point (shorter = more anomalous).
...
**LOF vs Isolation Forest:** LOF: better for local density anomalies (anomaly in a dense region that's less dense than its neighbors). Isolation Forest: better for global anomalies and high-dimensional data. LOF is O(n²) — slow for large data; IsoForest is O(n log n).
...
plt.axhline(y=8, color='red', linestyle='--', label='Cut here = 3 clusters')
plt.show()
...
Soft clustering — each point has probability of belonging to each cluster (unlike K-Means hard assignments).
...
**GMM vs K-Means:** GMM: elliptical clusters; soft probabilistic assignments; BIC/AIC for k selection. K-Means: spherical clusters, hard assignments, faster.

---

## 7.5. Clustering Embeddings at Scale (FAISS, Cosine vs L2)

Modern LLM/RAG pipelines work with **millions of dense embeddings**. Two production-critical adaptations:

### Use Cosine Similarity, Not L2 (for normalized embeddings)

```python
import numpy as np
from sklearn.preprocessing import normalize

emb_norm = normalize(emb, norm='l2', axis=1)   # unit-norm each row
# Now: cosine(a, b) = a · b  and  ||a − b||² = 2(1 − a·b)
# K-Means on normalized embeddings = spherical K-Means = cosine clustering
```

For DBSCAN / HDBSCAN, pass `metric='cosine'` directly.

### FAISS K-Means for Millions of Vectors

```python
import faiss
import numpy as np

emb = emb.astype("float32")                              # FAISS needs float32
emb_norm = emb / np.linalg.norm(emb, axis=1, keepdims=True)
kmeans = faiss.Kmeans(d=emb_norm.shape[1], k=200, niter=25, gpu=False, verbose=True)
kmeans.train(emb_norm)
centroids = kmeans.centroids                             # (k, d)

# Assign cluster to each point
_, cluster_ids = kmeans.index.search(emb_norm, 1)
cluster_ids = cluster_ids.ravel()
```

### Topic Clustering — BERTopic (the modern pipeline)

For text: BERTopic chains sentence-transformer → UMAP (dim reduction) → HDBSCAN (clustering) → c-TF-IDF (label clusters with keywords).
...
topic_model.get_topic_info()   # cluster labels + representative docs
```

Use cases: support ticket clustering, news topic discovery, log clustering, RAG corpus hygiene (find duplicate/near-duplicate clusters). Background on underlying embeddings → `../../4.nlp/02_embeddings/06_contrastive_training.md`
...
print(f"Silhouette: {sil:.3f}, CH: {ch:.3f}, DB: {db:.3f}")
```
...
**K-Means requires scaled features.** A feature with range [0, 10000] dominates distance calculation over a feature with range [0, 1]. Always StandardScale or MinMaxScale before K-Means, DBSCAN, LOF.

**t-SNE is for visualization only — not as preprocessing for a model.** t-SNE distances are not preserved, new points can't be transformed, and results change with random seed. Use PCA or UMAP for preprocessing.

**DBSCAN eps is not intuitive to set.** Always use the k-nearest-neighbor distance plot to guide eps selection. A wrong eps → everything is one cluster or all noise.

**Isolation Forest contamination must be set.** Default contamination=0.1 (10% outliers). If true outlier rate is different, calibrate. Use `contamination='auto'` to let the algorithm decide (less interpretable).

**K-Means local optima.** Different initializations → different solutions. Always use `n_init=10+` (run 10 times, keep best). K-Means++ reduces but doesn't eliminate this.

**Hierarchical scale.** O(n² log n) time and O(n²) memory — can't handle > 10K-100K samples. For large datasets, use K-Means, DBSCAN, or HDBSCAN.
...
|-----------|---------|-----|
| PCA preprocessing | `../01_fundamentals/03_feature_engineering.md` | Dimensionality reduction as feature preprocessing |
| K-Means as quantization | `../../2.deep learning/02_architectures/05_generative.md` | VQ-VAE uses K-Means-like discrete latent codes |
| GMM and EM algorithm | `04_probabilistic.md` | GMM trained with EM (Expectation-Maximization) |
| EM in depth | `08_expectation_maximization.md` | Full derivation, missing data, mixture models |
| Anomaly detection for documents | Your domain | Isolation Forest on OCR confidence scores + layout features |
| UMAP vs t-SNE | `../01_fundamentals/02_eda.md` | Visualization in EDA section |
| LOF and outlier handling | `../01_fundamentals/02_eda.md` | Outlier detection section |
| Embedding-based EDA | `../01_fundamentals/02_eda.md` | Apply these algorithms to embeddings |

---

## Key Takeaway

**Clustering:** K-Means (fast, spherical) → DBSCAN (arbitrary shape, finds outliers) → GMM (soft, elliptical) → t-SNE (visualization only, not preprocessing) → UMAP (visualization + preserves global structure).
