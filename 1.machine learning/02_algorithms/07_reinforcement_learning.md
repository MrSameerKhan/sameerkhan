...
```
         ...
         └──────────────────┬───────────────────────┘
                            │
                    Agent observes (s_t, r_t)
                    Agent chooses action a_t
                            │
         ┌──────────────────▼───────────────────────┐
         │              Agent                        │
         │  policy π: s → a                         │
         │  update: improve π from experience       │
         └──────────────────┬───────────────────────┘
                            │
                     action a_t
                            │
...
Expected cumulative reward from state s, taking action a, then following π:
```
Q^π(s, a) = E_π[G_t | s_t = s, a_t = a]
           = E_π[r_{t+1} + γ·V^π(s_{t+1}) | s_t = s, a_t = a]
```
...
Update Q((3,2), right):
  target = 10 + 0.9 · max Q((3,3), a')
         = 10 + 0.9 · 0   ← (3,3) is terminal, Q=0
         = 10
...
Q-learning learns a value function and derives policy from it (indirect).  
...
Discounted returns (from end):
  ...
  G₂ = 1.0 + 0.99 · 0.0  = 1.0
  G₁ = 1.0 + 0.99 · 1.0  = 1.99
  G₀ = 1.0 + 0.99 · 1.99 = 2.97

After normalization (mean=1.74, std=1.23):
  G₀_norm = (2.97 - 1.74) / 1.23 = +1.00
  G₁_norm = (1.99 - 1.74) / 1.23 = +0.20
  G₂_norm = (1.00 - 1.74) / 1.23 = -0.60

Policy at s₀: π(right|s₀) = 0.55 → log π = log(0.55) = -0.598

Loss contribution at t=0:
  -log π(a₀|s₀) · G₀_norm = -(-0.598) · 1.00 = +0.598

Gradient update:
  ∇_θ loss = +0.598 → θ updates to slightly increase π(right|s₀)
  (positive G₀ means "right" was a good choice here → increase its probability)

At t=2: G₂_norm = -0.60 → loss = -log π(right|s₂) · (-0.60) = negative
  Gradient update: slightly decrease π(right|s₂)
  (negative return means "right" at s₂ contributed to falling)
```
...
**RL applied to LLMs:**
- RLHF (Reinforcement Learning from Human Feedback): policy = LLM, reward = human preference score
- PPO updates LLM weights to maximize reward while staying close to original policy (KL penalty)
- See `6.llms/10_alignment_end_to_end.md` for full trace
...
A: ∇_θ J(θ) = E_π[∇_θ log π_θ(a|s) · G_t]. It says: to increase expected return, nudge policy parameters in the direction that increases the log probability of actions that led to high returns, and decreases it for low-return actions. The log trick avoids needing to differentiate through the environment.
...
- **RLHF in LLMs:** `6.llms/10_alignment_end_to_end.md`
