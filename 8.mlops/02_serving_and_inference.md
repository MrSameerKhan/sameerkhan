...
PagedAttention: KV cache in fixed-size pages (like OS virtual memory)
  ...
  → Share pages between sequences that have common prefix
...
# View in TensorBoard: tensorboard --logdir ./profiler_log
```
...
A: Step-by-step: (1) Export to ONNX with dynamic axes → framework-agnostic, ONNX Runtime optimized execution; (2) Apply onnxruntime-transformers optimizer — fuses attention patterns, removes redundant operations; (3) FP16 conversion — halves memory and speeds up GPU compute; (4) Dynamic quantization to INT8 for CPU deployment; (5) Profile with torch.profiler to find actual bottlenecks; (6) Server-side dynamic batching — collect requests within 10ms window, process as a batch; (7) Horizontal scaling with load balancer for high concurrency. Typical result: 4-8× latency improvement over vanilla PyTorch.

**Q: What is PagedAttention in vLLM and why does it matter?**
A: Standard LLM serving pre-allocates contiguous memory for each sequence's KV cache up to the maximum sequence length — most of this is wasted for short responses and limits concurrent requests. PagedAttention stores KV cache in fixed-size non-contiguous pages (like OS virtual memory), allocating pages dynamically as the sequence grows. This eliminates internal fragmentation (pre-allocated but unused space) and enables: (1) more concurrent sequences per GPU, (2) prefix caching — sequences sharing a common prefix (e.g., same system prompt) can share KV cache pages, (3) continuous batching — new requests can start as soon as a slot frees, not waiting for the whole batch to finish. Result: 24× higher throughput vs naive HuggingFace on an A100.

---

## Connections
- **Efficient Transformers (transformers/models/04):** Quantization, Flash Attention covered architecturally
- **MLOps Monitoring (7.mlops/03):** Monitor latency/throughput metrics post-deployment
...
## Key Takeaway
Serving stack: ONNX export → FP16 → ONNX Runtime for BERT-class models. vLLM for LLMs — PagedAttention gives 24× throughput. FastAPI for REST API with dynamic batching. Always profile before optimizing — bottleneck is rarely where you expect. Key metrics: P95 latency and throughput (req/s). SLA targets: <50ms for classification, <2s TTFT for LLM.
