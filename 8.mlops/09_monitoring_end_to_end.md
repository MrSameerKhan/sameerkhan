# Monitoring & Drift Detection — End-to-End
...
class PredictionLogger:
    ...
    def flush(self):
        if not self.buffer:
            return
        key = f"{self.prefix}/{datetime.utcnow().strftime('%Y/%m/%d/%H')}/{uuid.uuid4()}.jsonl"
...
Training data (Jan 2023):
  invoice_amount: mean=$1,240, std=$890, range=[50, 45,000]

April 2024 serving data:
  invoice_amount: mean=$1,890, std=$1,420, range=[80, 95,000]
...
PSI calculation (10 bins):
  Bin [50–500]:   ref=32%, cur=18% → (0.18-0.32)×log(0.18/0.32) = -0.14×(-0.575) = 0.081
  Bin [500–1500]: ref=28%, cur=25% → small contribution = 0.004
  Bin [1500–5000]:ref=22%, cur=31% → positive contribution = 0.034
  Bin [5000–45K]: ref=18%, cur=26% → positive contribution = 0.041
...
class AlertManager:
    ...
    def check_and_alert(self, metrics: dict, drift_results: dict):
        ...
        # 1. AUC drop
        if self.baseline_auc and metrics["auc"] < self.baseline_auc - self.thresholds.auc_regression_pct:
            alerts.append({
                "severity": "critical",
                "message":  f"AUC dropped from {self.baseline_auc:.3f} to {metrics['auc']:.3f} "
            ...
            alerts.append({
                "severity": "warning",
                "message":  f"AUC {metrics['auc']:.3f} below threshold {self.thresholds.auc_min}",
        ...
        for feature, result in drift_results.items():
            if result.get("psi", 0) > self.thresholds.psi_critical:
                alerts.append({
                    "severity": "critical",
                    "message":  f"Critical drift on feature '{feature}': PSI={result['psi']:.3f}",
                    "action":   "RETRAIN TRIGGER",
                })
            elif result.get("drifted"):
                alerts.append({
                    "severity": "warning",
                    "message":  f"Drift detected on feature '{feature}': p={result['p_value']:.4f}",
                    "action":   "Monitor closely",
                })

        # 3. Prediction rate shift (proxy for concept drift when no labels yet)
        if abs(metrics["pred_rate"] - metrics.get("baseline_pred_rate", 0.15)) > self.thresholds.pred_rate_change:
            alerts.append({
                "severity": "warning",
                "message":  f"Prediction rate shifted to {metrics['pred_rate']:.3f} "
                            f"(was {metrics.get('baseline_pred_rate', 0.15):.3f})",
...
def run_daily_monitoring():
    ...
    if len(labeled) >= 100:
        metrics = {
            ...
            "f1":               f1_score(labeled["label"], (labeled["prediction"] > 0.42).astype(int)),
...
```
Day 0 (model deployed): baseline AUC = 0.923, pred_rate = 0.148
...
Day 30 monitoring run:
  ...
  PSI (invoice_amount): 0.160  (minor drift, warning sent to Slack)
...
Day 60 monitoring run:
  ...
  pred_rate: 0.201  (change = +0.053 > 0.05 threshold → label drift warning)
  PSI (invoice_amount): 0.234  (PSI > 0.20 → TRIGGER)
  → Slack:  🔴 AUC regression 0.923 → 0.871. RETRAIN TRIGGER.
  → Slack:  🔴 Critical drift on invoice_amount PSI=0.234. RETRAIN TRIGGER.
...
A: Data drift (covariate shift): input distribution P(X) changed — e.g., new invoice formats, new currencies. Model may still be correct, but inputs are out-of-distribution. Concept drift: the relationship P(Y|X) changed — e.g., fraud patterns evolved. Even inputs that look the same now have different correct labels. Data drift is detectable without labels (compare feature distributions). Concept drift requires labels to detect directly, but prediction rate shifts can be an early proxy.

**Q: How do you detect drift without ground truth labels?**
A: Three approaches: (1) Input feature monitoring — compare serving feature distributions to training reference using KS test or PSI per feature; (2) Prediction rate monitoring — track fraction of positive predictions; a sudden shift suggests the model is seeing different inputs; (3) Embedding drift — if using a neural model, track the distribution of intermediate embeddings (e.g., last layer activations) using distance metrics. Labels are only needed for performance metrics (AUC, F1).

**Q: When should you retrain vs recalibrate?**
A: Recalibrate (adjust decision threshold) when: prediction rate shifted but AUC is stable. The model still ranks items correctly, just the calibration is off — adjust threshold from 0.42 to 0.55. Retrain when: AUC drops (model is no longer discriminating well), or significant input drift (new patterns the model hasn't seen). Recalibration is cheap (no GPU needed); retrain is expensive — use recalibration as a quick fix while retraining runs.

---

## Connections

- **Model registry:** `8.mlops/08_model_registry_end_to_end.md` — rollback when monitoring triggers
- **Serving:** `8.mlops/02_serving_and_inference.md` — logging predictions from serving layer
- **Pipelines:** `8.mlops/04_pipelines_and_infra.md` — retraining DAG triggered by monitoring
- **Reference theory:** `8.mlops/03_monitoring_and_drift.md` — statistical tests, PSI explained

## Key Takeaway

Monitor three things: (1) **data drift** — compare input feature distributions to training reference using KS test + PSI; (2) **performance metrics** — AUC/F1 on labeled window; (3) **prediction rate** — free early-warning signal before labels arrive. Alert on: AUC regression >3%, PSI >0.20, prediction rate shift >5%. Retrain when performance drops or drift is critical; recalibrate threshold when AUC is stable but calibration shifted. Ground truth latency is the hardest practical challenge — most production systems can only detect input drift in real time.
