# On-Premise ML Model Deployment — End to End

## What This Covers

A remote client device (phone, laptop, another server) sends a request → load balancer → inference server → ML model → response back to client.

Every step from bare metal to production-ready serving.
...
Before installing anything, understand your hardware:
...
# Check what driver version supports your GPU:
...
# | 0    NVIDIA A100 80GB  Off         | 00000000:01:00.0 Off  |              0  |
# |  0%   35C  P0   55W /400W |  0MiB / 81920MiB |      2%      Default        |
# +-----------------------------------------------------------------------------+

# ─── Install CUDA Toolkit ─────────────────────────────────────────────────
# Match CUDA version to your driver version (nvidia-smi shows compatible CUDA)
# CUDA 12.1 compatible with driver 525+
...
# PyTorch with CUDA 12.1 (match your CUDA version)
...
sess = ort.InferenceSession("/opt/ml_serving/models/bert-sentiment/model.onnx",
                             providers=["CUDAExecutionProvider"])
print("ONNX inference works:", sess.run(None, {
...
# Port 8000 is NOT added → only nginx on localhost can reach it
...
Step 3 — FastAPI receives request
  ...
  Pydantic validates body: {"texts": ["cat sat on mat"]} ✓
...
Step 5 — Model Inference
  ...
  torch.softmax([-0.83, 1.24]) → [0.14, 0.86]

Step 6 — Postprocessing
  argmax([0.14, 0.86]) = 1 → "positive"
  confidence = 0.86

Step 7 — Response built
  {
    "predictions": [{"text": "cat sat on mat", "label": "positive", "confidence": 0.86, "inference_time_ms": 4.2}],
    "total_time_ms": 5.1,
    "device": "cuda"
  }
...
Problem: Server crashes after hours of use
  Reason: GPU memory leak (gradients accumulating)
...
## 7. Production Gaps — What Was Missing Above

The steps above cover the skeleton. Below are all the production-critical details a real deployment needs.
...
network:
  version: 2
  ethernets:
    ens3:                          # your NIC name (check with: ip link show)
      dhcp4: false
      addresses:
...
ssh-keygen -t ed25519 -C "sameer@company.com" -f ~/.ssh/ml_server_key

# Copy public key to server
ssh-copy-id -i ~/.ssh/ml_server_key.pub ubuntu@192.168.1.100
...
ssh -i ~/.ssh/ml_server_key ubuntu@192.168.1.100   # should work
...
# sk-AbCdEfGhIjKlMnOpQrStUvWxYz012345678901234567

# Set in environment (never hardcode in code)
echo 'API_KEYS=sk-AbCdEfGhIjKlMnOpQrStUvWxYz012345678901234567,sk-otherkey' \
    >> /opt/ml_serving/.env
```
...
MAX_BATCH_SIZE=32
MAX_SEQ_LENGTH=128
API_KEYS=sk-AbCdEfGhIjKlMnOpQrStUvWxYz012345678901234567
LOG_LEVEL=INFO
...
The first inference after startup is always slow — CUDA compiles kernels, allocates memory, JIT compiles ops. Warm up the model during startup so users don't see the slow first request.

```python
# Add to server.py — runs at startup before serving traffic
...
# Warm-up pass 1: 312.4ms   ← slow (CUDA kernel compilation)
...
Instead of processing each request as it arrives, collect requests over a short window (e.g., 20ms) and process them as one batch. This dramatically improves GPU utilization.
...
With dynamic batching (20ms window):
  ...
  50 batches × (model forward pass) = 50 × 16ms = 800ms GPU time
...
gunicorn server:app \
    ...
    --timeout 60               # kill worker if no response in 60s

# Nginx side: before restarting inference server, drain it from load balancer
# Nginx upstream: mark server as down
# 1. Remove server from nginx upstream (or set weight=0)
# 2. Wait for in-flight requests to drain (check access logs)
# 3. Restart inference server
# 4. Re-add to nginx upstream
```

```bash
# Manual graceful restart sequence
# Step 1: Tell nginx to stop sending new traffic to server-1
sudo nginx -s reload   # after editing upstream to remove server-1
...
When you have a new model version, you want to deploy it without any downtime.
...
Blue-green steps:
  ...
  Step 2: Health check passes on 8001
...
Without log rotation, inference logs will fill the disk within days.

```bash
# /etc/logrotate.d/ml-inference
...
sudo tee /etc/cron.d/model-backup << 'EOF'
0 2 * * * ubuntu rsync -avz --delete \
...
Before going live, stress test to find breaking point.
...
# POST/predict 4820    0     12ms  28ms 45ms 40.2
# GET /health   480    0      2ms   4ms  6ms   4.0
...
On-premise ML deployment = hardware (GPU + drivers + CUDA) → Python env → model (save locally, optional ONNX export) → FastAPI inference server (Uvicorn/Gunicorn) → systemd (auto-start) → Nginx (TLS + load balancing + rate limiting) → firewall (block direct GPU server access) → monitoring (Prometheus + Grafana). The request flow: client → HTTPS → Nginx (TLS termination, round-robin) → HTTP → FastAPI → tokenize → GPU forward pass → JSON response → back to client. Docker wraps everything for reproducibility. Critical details: one worker per GPU, inference_mode for speed, health endpoint for load balancer, GPU memory monitoring to catch OOM before it crashes production.

**Production extras that most guides skip:** static IP + NTP + hostname setup, SSH key-only + fail2ban + UFW, API key auth on FastAPI, .env secrets management, model warm-up (3 passes), dynamic batching (20ms window), graceful shutdown (drain in-flight), blue-green deployment (zero downtime), model quantization (fp16 → 2× speed), log rotation (daily, 14 days), dedicated NVMe disk, backup with checksums, load testing to find breaking point.
