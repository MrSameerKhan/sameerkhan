# 04 Document Processing Pipeline Design

## Problem Statement

Design an end-to-end document processing pipeline for a company that receives 100K documents/day (invoices, contracts, receipts) and needs to extract structured data, classify document types, and route for processing. Latency: < 30 seconds per document. Accuracy: > 95% field-level precision.
...
class DocumentClassifier:
    ...
    def classify(self, text: str = None, image: Image.Image = None,
...
class ExtractionEngine:
    ...
    def extract(self, image: Image.Image, doc_type: str) -> dict:
        ...
        return rule_result.result_dict()
    ...
    def rule_based_extract(self, text: str, doc_type: str) -> BaseModel:
        ...
        patterns = {
            "invoice_number": r"(?:invoice|inv|bill)\s*(?:no|number|#)?\s*:?\s*([A-Z0-9-]{3,}/]+)",
            "date": r"(?:invoice\s)?date\s*:?\s*(\d{1,2}[/\-]\d{1,2}[/\-]\d{2,4}|\w+\s+\d{1,2},?\s+\d{4})",
...
class ConfidenceScorer:
    ...
    def score(self, extracted_data: dict, doc_type: str, ocr_confidence: float) -> dict:
        ...
        important_fields = {"invoice_number": 0.3, "date": 0.2, "total_amount": 0.3,
                            "vendor_name": 0.2}
        overall = sum(
            field_scores.get(f, 0) * w for f, w in important_fields.items()
        )
    ...
    def _score_field(self, field: str, value: str, doc_type: str) -> float:
        validators = {
            ...
            "invoice_number": lambda v: 1.0 if re.match(r'[A-Za-z0-9-/]{3,20}', str(v)) else 0.6,
        }
...
class HumanReviewQueue:
    ...
    def queue_for_review(self, doc_id: str, extracted_data: dict, confidence_report: dict):
        review_item = {
            ...
            "review_fields": [
                f for f, s in confidence_report["field_scores"].items() if s < 0.8
            ],
        ...
        with ThreadPoolExecutor(max_workers=num_workers) as executor:
            while True:
                ...
                for msg in messages:
                    ...
                    futures.append((executor.submit(self.process_message, body), receipt_handle))
...
```
Throughput:
  100K docs/day = ~1.2 docs/second average
  Peak: 10x = 12 docs/second
  Workers needed: 12 docs/s × 15s/doc = 100 concurrent workers (scale with k8s)
...
**Q: How would you handle a 10x spike in document volume?**
