...
# Classical pipeline
print(clean_text("Hello!!! <br> Visit https://example.com 🚀", task='classical'))
# → "hello visit"

# Transformer pipeline (minimal)
print(clean_text("Hello!!! <br> Visit https://example.com 🚀", task='transformer'))
# → "Hello!!! Visit 🚀"
```

**When to keep noise:**
- Emojis: keep for sentiment analysis (😊 = positive signal)
- Numbers: keep for financial text, dates in documents
- Punctuation: keep for transformers (period = sentence boundary signal)
...
```
"play", "playing", "played", "player" → 4 different tokens → huge vocabulary
"unhappiness" → unseen in training → OOV (out-of-vocabulary) → UNK token
```

### Subword Tokenization — The Modern Standard

Subword tokenization splits rare words into meaningful pieces. Common words stay whole.
...
### Special Tokens
```
[CLS]: classification token — BERT prepends this, its hidden state = sentence representation
...
Remove high-frequency words with low semantic content.
...
**When to remove:**
- TF-IDF + classical ML: yes — reduces dimensionality, focuses on content words
- Topic modeling (LDA): yes — topics shouldn't include "the", "is"
- Information retrieval / search: sometimes — depends on query type

**When NOT to remove:**
- Transformers (BERT, GPT): never — attention uses full context; "not good" ≠ "good"
- Sentiment analysis with negation: "not happy" → removing "not" flips meaning
- Any task where grammatical structure matters
...
**Why sentence segmentation matters:**
- Many models have max token limits (BERT: 512, GPT: 2048-128K) → need to chunk by sentence
- Machine translation: translate sentence by sentence
- Summarization: sentence-level scoring (ROUGE)
...
def normalize_document_text(text: str) -> str:
    ...
    text = text.replace('0', 'O') if text.isupper() else text  # zero/O confusion
    ...
    # Normalize currency
    text = re.sub(r'[\$€£]\s*(\d)', r'CURRENCY \1', text)

    # Normalize dates
    text = re.sub(r'\b(\d{1,2})[/\-](\d{1,2})[/\-](\d{2,4})\b', r'DATE', text)
...
**Uses in NLP pipeline:**
- Lemmatization needs POS ("running" → "run" only if it's a verb, not "runner")
- Feature extraction for classical NLP
- Dependency parsing for information extraction
- Chunking noun phrases for NER preprocessing
...
# batch['input_ids']: [2, 128] tensor
# batch['attention_mask']: [2, 128] tensor (1=real token, 0=padding)
```
...
| Stemming | ✅ Sometimes | ❌ Never | Use lemmatization instead |
...
Lowercasing before stemming, tokenizing before stop word removal. Wrong order → incorrect results. Example: lemmatizer needs POS tag; POS tagging needs the full sentence.

**Don't apply training-set stop words to test set.**
If you build a custom stop word list from training data, apply it consistently to both train and test. Using sklearn's `TfidfVectorizer` with `stop_words='english'` handles this correctly.

**Subword tokenizers add special tokens — account for this in max_length.**
BERT adds [CLS] and [SEP] → 512 token limit = 510 actual tokens. For sentence pairs, both [SEP] tokens count. Truncation strategy matters: `truncation='longest_first'` for sentence pairs.

**Padding side matters for causal (autoregressive) models.**
GPT-family: left-pad (pad on left, tokens on right), so the actual text ends at position -1 and the model's causal attention sees text correctly. BERT: right-pad is fine.
...
A: BPE (Byte-Pair Encoding) is a subword tokenization algorithm that iteratively merges the most frequent adjacent character pairs until a fixed vocabulary size is reached. It's used because: (1) fixed vocabulary handles any word including OOV — "ChatGPT" splits to known subwords rather than [UNK]; (2) morphologically rich languages (Finnish, Turkish) are handled better since word variants share subword tokens; (3) multilingual models benefit because common morphemes ("re-", "-ing") are shared across languages; (4) vocabulary is compact — 50K tokens cover most languages reasonably. The tradeoff: tokenization is now part of the model and must be consistent between training and inference.

**Q: When would you remove stop words and when would you keep them?**
A: Remove for: classical ML pipelines (BoW/TF-IDF + Logistic Regression/SVM), topic modeling (LDA), keyword extraction — stop words add dimensions without signal. Keep for: any transformer model (BERT, GPT) — self-attention uses the full sequence including "not", "is", "the" as context; negation-sensitive tasks ("not good" vs "good"); grammatical structure tasks (parsing, translation); any time sentence meaning depends on function words. The safest rule: keep stop words unless you have specific evidence they hurt performance in your classical pipeline.

**Q: A BERT model on a long document (2000 tokens) — how do you handle the 512 limit?**
A: Several strategies: (1) **Truncation** — simplest, loses information from end; best when answer is typically near the start. (2) **Sliding window** — process document in overlapping 512-token windows, aggregate predictions (max pool, vote, or use CLS from each window). (3) **Hierarchical model** — split into sentences, encode each separately with BERT, then apply another model (LSTM or transformer) over sentence embeddings. (4) **Long-context models** — Longformer (4096 tokens, sparse attention), BigBird (4096 tokens), or modern LLMs (LLaMA-3 with 128K context). For document extraction (your domain), sliding window with overlap=50 tokens and taking the highest confidence prediction across windows is standard practice.
...
| BERT tokenizer details | `../../4.transformers/` | Transformer-specific preprocessing |
...
**Classical pipeline:** clean → lowercase → tokenize → remove stop words → lemmatize → vectorize (TF-IDF).
