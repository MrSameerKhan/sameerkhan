...
| MCP | Model Context Protocol — standardized tool/context integration (Anthropic, 2024) |
| Computer Use | LLM controls a virtual desktop (clicks, types, scrolls) — Claude Computer Use, OpenAI Operator |
| Memory | Short-term (context window), long-term (vector DB or key-value store) |

---

## 2024-2025 Agent Frameworks

| Framework | Style | Best for |
|-----------|-------|----------|
| LangGraph (LangChain) | Explicit state graph (nodes + edges + state schema); supersedes the older imperative LangChain Agent class | Production agents — replayable, debuggable, persistable state |
| CrewAI | Role-based (each agent has a role + goal + backstory); task decomposition | Multi-agent collaboration with clear specialization |
| AutoGen (Microsoft) | Multi-agent conversation; supports group chat, function-callers, code-runners | Research, multi-agent dialogues, code-heavy workflows |
| OpenAI Swarm | Lightweight, agent + handoff primitives; reference design rather than full framework | Simple multi-agent flows on OpenAI models |
| smolagents (HuggingFace) | Code-as-action — agents write Python instead of JSON tool calls | When you want code-execution as the universal tool |
| Pydantic AI | Type-safe agent framework built on Pydantic | Production agents needing strong typing + structured I/O |
| LlamaIndex Agents | Tighter RAG integration; query engine + tool agents | Heavy retrieval workflows |
| Anthropic Computer Use / OpenAI Operator | Computer Use API — control a desktop via vision + actions | Browser automation, GUI tasks where no API exists |

**2025 default for production:** LangGraph (state machine model maps cleanly to ops needs — checkpointing, human-in-the-loop, parallel branches). For research / prototyping: AutoGen or smolagents. For tight RAG: LlamaIndex.

Reliability patterns (retry, validation, loop detection, tool-call deduplication, structured-output enforcement, HITL approval, audit log) covered in depth: `02_agent_reliability_patterns.md`.
Multi-tenant + security patterns: `../11.system_design/09_tool_authorization_patterns.md`
...
memory.save_context(
    ...
    {"output": "Noted! I'll remember Alice works at Anthropic."}
)
...
def save_episode(query, task, result, tools_used, duration):
...
**Architecture:** Host (Claude app) ↔ MCP Client ↔ MCP Server ↔ External tool/data
...
def get_document(doc_id: str) -> str:
    """Exposes documents as resources the model can read."""
...
def search_documents(query: str, doc_type: str = None) -> list[dict]:
    ...
    return search_db(query, filters={"type": doc_type} if doc_type else None)
...
Simple LLM inference is a single forward pass: prompt → response. An agent is an LLM embedded in a loop where it can observe its environment, decide to take actions (call tools), receive observations from those actions, and continue reasoning until the task is complete. The key capabilities: planning (deciding what to do), tool use (executing external actions), and memory (maintaining state across steps). Agents are appropriate when tasks require multiple steps, external information, or side effects (writing files, sending emails, querying databases).

**Q: Explain the ReAct pattern and why it works.**

ReAct (Reason + Act) structures LLM outputs into explicit "Thought — Action — Observation" cycles. The model writes its reasoning (Thought), specifies a tool call (Action), receives the tool result (Observation), and repeats. This works because: (1) reasoning traces reduce hallucination by grounding inference in observations, (2) the model can plan next steps based on actual tool outputs rather than imagined outcomes, (3) it provides a natural debugging surface — you can see exactly where the agent went wrong. The original paper showed ReAct significantly outperforms chain-of-thought (reasoning only) and acting-only baselines on interactive tasks.
...
Agents = LLM + tools + memory + loop. ReAct pattern: Thought → Action → Observation → repeat. MCP standardizes tool integration — build once, use everywhere. Key reliability concerns: infinite loops (add `max_iterations`), tool hallucination (validate inputs with Pydantic), cost explosion (token + turn budgets). Memory: in-context for short sessions; recall, key-value for structured facts.
