...
──────────────────────────────────────────────────
"INVOICE"       [50, 30, 180, 60]        0.99
"NUMBER:"       [185, 30, 310, 60]       0.98
"INV-2024-0432" [315, 30, 500, 60]       0.97
"DATE:"         [50, 80, 120, 110]       0.99
"2024-03-14"    [125, 80, 270, 110]      0.98
"VENDOR:"       [50, 130, 145, 160]      0.99
"Acme"          [150, 130, 220, 160]     0.97
"Corp"          [225, 130, 295, 160]     0.97
"TOTAL:"        [50, 400, 130, 430]      0.99
"$1,250.00"     [680, 400, 800, 430]     0.98
```
...
"INVOICE":        [50,30,180,60]   → [50, 37, 180, 75]
"INV-2024-0432":  [315,30,500,60]  → [315, 37, 500, 75]
"DATE:"           [50,80,120,110]  → [50, 100, 120, 137]
"2024-03-14"      [125,80,270,110] → [125, 100, 270, 137]
"$1,250.00"       [680,400,800,430]→ [680, 500, 800, 537]
...
5. Width/Height embedding: x1-x0 and y1-y0 (via nn.Embedding(1001, 128))

Layout embeddings are projected from 128 → 768 and added to token embedding.

Final embedding for "2024-03-14":
  token_emb("2024") + pos_emb(13) + x_emb(125) + x_emb(270) 
...
Attention scores:
  A = QK^T / √64   (248×64) · (64×248) / 8 = (248×248)

Let's trace attention for "2024-03-14" (token index 13) attending to others:

  score(13, 4)  = Q[13] · K[4]  / 8   → "date value" attending to "DATE:" label
  score(13, 7)  = Q[13] · K[7]  / 8   → attending to invoice number (same row? No.)
  score(13, 196+) = visual patches at same spatial location
...
"NUMBER:"     [-1.8, 0.1, -0.9, ...]   O                  0.79
"INV"         [0.3, 3.8, -2.1, ...]    B-INVOICE_NUM      0.94
"-"           [-0.2, 0.8, 3.2, ...]    I-INVOICE_NUM      0.87
"2024"        [-0.1, 0.6, 3.1, ...]    I-INVOICE_NUM      0.85
"-"           [-0.3, 0.7, 3.0, ...]    I-INVOICE_NUM      0.84
"0432"        [-0.2, 0.5, 3.3, ...]    I-INVOICE_NUM      0.88
"DATE:"       [-2.0, 0.2, -1.1, ...]   O                  0.81
"2024"        [0.2, -1.5, -0.8, 3.9, ...]  B-DATE         0.91
"-"           [-0.3, -1.2, -0.6, 0.7, 3.7, ...] I-DATE   0.89
"03"          [-0.2, -1.1, -0.5, 0.5, 3.8, ...] I-DATE   0.90
"$1"          [-2.1, -1.8, -1.5, -0.9, -0.3, -0.2, 0.1, 4.1, -0.7] B-TOTAL  0.93
",250"        [same pattern]            I-TOTAL            0.90
...
"INV"       B-INV (cl 1)  [0.03, 0.94, 0.01, ...]            -log(0.94) = 0.062
"-"         I-INV (cl 2)  [0.05, 0.08, 0.87, ...]            -log(0.87) = 0.139
"2024"      I-INV (cl 2)  [0.04, 0.11, 0.85, ...]            -log(0.85) = 0.163
"DATE:"     O (class 0)   [0.79, 0.04, 0.05, ...]            -log(0.79) = 0.236
"2024"      B-DATE (cl 3) [0.02, 0.03, 0.04, 0.91, ...]      -log(0.91) = 0.094

Average loss = (0.198 + 0.062 + 0.139 + 0.163 + 0.236 + 0.094) / 6 = 0.149
...
Test F1: 0.884 (on held-out 50 invoices)
```
...
def prepare_example(example):
    ...
    example = {
        "image": PIL.Image,
        "words": ["INVOICE", "NUMBER:", "INV-2024-0432", ...],
...
─────────────────────────────────────────────────────────────────
v1       2020      Text + 2D position embeddings          113M
                   No image, just layout-aware BERT

v2       2021      + Image features (ResNeXt backbone)    200M
                   + Spatial-aware self-attention
                   Text and image via cross-attention

v3       2022      Unified multimodal pre-training         133M (base)
                   + MIM (Masked Image Modeling)                368M (large)
                   + Word-patch alignment objective
                   Same self-attention for text AND image
                   (no separate cross-attention)
                   → simpler, more effective, smaller
...
```
Each word from OCR gets its own bounding box. Subword tokens from tokenization
inherit their parent word's bbox. So "Acme Corp Ltd" → 3 bboxes (one per word).
During fine-tuning, the model labels each subword token separately.
Post-processing collapses consecutive B-/I- tokens of the same type into one field.
```
...
Rule of thumb:
  Clean, typed documents at scale → LayoutLM v3
  Noisy, varied, handwritten, or low training data → Donut or GPT-4V
```
...
Annotation process:
  ...
  3. Annotate each word with IOB label using Label Studio or Prodigy
     (click on "invoice number" field, draw box or select tokens)
  4. Export: {"words": [...], "boxes": [...], "labels": [...], "image": path}

Weak supervision shortcut:
  Use regex to auto-label easy patterns:
...
Active learning:
  Train on 100 examples → predict on unlabeled pool
  → find lowest-confidence examples → annotate those → retrain
  Reaches 90% of full-data performance with 30% of the labels.
```
...
Production monitoring:
  ...
  Per-vendor accuracy (new vendors → lower accuracy → retrain signal)
...
Input:  OCR words + normalized bboxes [0,1000] + document image (224×224 → 196 patches)
Encode: token_emb + position_emb + layout_emb (x,y,w,h each have embeddings)
Fuse:   12-layer self-attention — text tokens and image patches attend to each other
Output: contextual embedding per token → linear head → IOB label probabilities

Training:  Cross-entropy on IOB labels, only text tokens (image patches ignored)
           5e-5 lr, 20 epochs, batch 4, FP16 — ~2hr on A100 for 500 docs
...
v3 improvements over v2:
  ...
  - Fewer params (133M vs 200M), better results
