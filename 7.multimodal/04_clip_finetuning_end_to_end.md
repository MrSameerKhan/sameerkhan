...
Step 5: 12-layer transformer (ViT-B/32)
  Self-attention: 12 heads × 64 dims/head = 768 total
  FFN: 768 → 3072 → 768
...
```
Mini-batch: N=4 image-text pairs (N=32768 in actual CLIP training)
...
Image embeddings (after L2 normalization, each ∈ R^512):
  I₁ = image_encoder(invoice_image)   = [0.12, -0.34, 0.07, ...]
  I₂ = image_encoder(dog_photo)       = [-0.45, 0.23, 0.18, ...]
  I₃ = image_encoder(contract_image)  = [0.08, -0.29, 0.11, ...]
...
Text embeddings (after L2 normalization, each ∈ R^512):
  ...
  T₂ = text_encoder("A brown labrador dog running")    = [-0.44, 0.22, 0.17, ...]
...
I₂(dog)      [ 0.04,    0.89,    0.06,        0.05  ]
I₃(contract) [ 0.39,    0.05,    0.88,        0.35  ]
I₄(receipt)  [ 0.36,    0.04,    0.33,        0.91  ]

Diagonal = matching pairs → HIGH similarity (0.89-0.92)
...
Image→Text direction (row-wise softmax):
  ...
  softmax([13.16, 0.43, 5.86, 5.43]):
    ...
    P(T₁|I₁) = 513,340 / 513,920 = 0.9989
    L_I1 = -log(0.9989) = 0.0011
  ...
  softmax([5.58, 0.71, 12.58, 5.00]):
    P(T₃|I₃) = exp(12.58) / sum ≈ 0.9970
    L_I3 = 0.0030
...
Fine-tune when:
  ...
  - Large labeled dataset available (1K+ examples)
...
Train only a linear classifier on top of CLIP features.
...
Learning rate schedule:
  ...
  Classifier head: lr = 1e-4 (10-100× larger — train from scratch)
...
Use case: specialized domain with descriptive text available
  Documents: (invoice_image, "Invoice from Acme Corp, total $1250, date 2024-03-14")
...
doc_classes = {
    ...
    "purchase_order":   ["a purchase order form", "a procurement document"],
...
# {"invoice": 0.71, "contract": 0.08, "receipt": 0.05, ...}
```
...
```
CLIP creates an N×N similarity matrix where N is batch size.
Row i = image i compared to all N texts.
Column j = text j compared to all N images.

The diagonal contains matching pairs (correct answers).
We apply cross-entropy loss row-wise (image→text direction):
  "given image i, which text j is the match?" → correct answer = diagonal[i]
And column-wise (text→image direction).
Total loss = average of both directions.

The key ingredient: large N (32,768 in original paper).
Large batch = many negative pairs = harder task = better representations.
Temperature τ sharpens/softens the softmax — CLIP learns τ jointly.
```

**Q: How does CLIP enable zero-shot classification?**
```
During pre-training on 400M image-text pairs, CLIP learns that
"a photo of a dog" is close to dog images, "a receipt" is close to receipt images, etc.

At inference: create one text embedding per class ("a photo of X"),
then find which class embedding is closest to the test image embedding.
No labeled examples needed — the knowledge comes from pre-training.

Zero-shot works well when the class was well-represented in training data.
Fails for rare/specialized domains (satellite imagery, medical scans, documents).
```
...
```
The contrastive loss requires negative examples. Each image is compared against
N-1 texts in the batch. More negatives → harder task → model must learn finer
distinctions → better representations.

With N=256: each image has 255 negatives — easy to distinguish
With N=32,768: each image has 32,767 negatives — must distinguish very similar concepts

The "hardness" of negatives determines representation quality.
Large batch approximates having all possible negatives in every update.

Practical implication: reproducing CLIP from scratch requires TPU pods.
Most practitioners fine-tune the pre-trained CLIP rather than pre-train from scratch.
```
