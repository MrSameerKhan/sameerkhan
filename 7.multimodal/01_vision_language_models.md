...
**Architecture:**
```
Image → [ViT or ResNet] → image_embedding ∈ ℝᵈ
Text  → [Text Transformer] → text_embedding ∈ ℝᵈ
...
InfoNCE loss (symmetric):
  L_img→txt = -1/N Σᵢ log exp(sᵢᵢ/τ) / Σⱼ exp(sᵢⱼ/τ)
  L_txt→img = -1/N Σᵢ log exp(sᵢᵢ/τ) / Σⱼ exp(sⱼᵢ/τ)
...
print({cls: prob.item() for cls, prob in zip(classes, probs[0])})
# {"a cat": 0.92, "a dog": 0.05, "a bird": 0.02, "a car": 0.01}
```
...
```python
# Extract CLIP features → train logistic regression
# Outperforms many fully supervised models on few-shot benchmarks
...
# Train logistic regression on frozen CLIP features
le = LabelEncoder()
...
**CLIP embeddings are NOT interchangeable with sentence embeddings.** CLIP is trained for image-text alignment — its text embeddings work well for image retrieval but are inferior to SBERT for text-only semantic similarity tasks.
...
- **Transfer Learning (CV/applications/01):** CLIP features as frozen backbone for few-shot tasks
