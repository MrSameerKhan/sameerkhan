...
|-----------|------|---------|
| Scaled Dot-Product Attention | Match queries to keys, weight values | softmax(QKᵀ/√d_k)V |
| Multi-Head Attention | Attend to multiple subspaces in parallel | Concat(head₁,...,headₕ)Wᴼ |
| Positional Encoding | Inject sequence order (no recurrence) | sin/cos at different frequencies |
| FFN (Feed-Forward) | Per-position nonlinear transform | max(0, xW₁+b₁)W₂+b₂ |
...
RNNs process tokens sequentially → O(N) steps, gradients must traverse full sequence → long-range forgetting.
CNNs have fixed receptive fields → struggle with long-range dependencies.
...
This is **pre-norm** (modern standard — GPT-2, LLaMA, Mistral).
Original "Attention is All You Need" used post-norm — harder to train deep without careful warmup.
...
scores = Q · Kᵀ / √d_k      # [seq_len, seq_len] — raw compatibility
weights = softmax(scores)    # [seq_len, seq_len] — rows sum to 1
output = weights · V         # [seq_len, d_v]
```

**Why √d_k?**
Without scaling, dot products grow with d_k (variance = d_k if Q,K ~ N(0,1)) → softmax saturates into near-one-hot → gradients vanish. Dividing by √d_k keeps variance ≈ 1.
...
- Trainable embedding table for each position
- Works well but doesn't generalize to unseen sequence lengths

**Rotary (RoPE — LLaMA, Mistral, GPT-NeoX):**
- Encodes relative position via rotation of Q,K before attention
- Better length generalization, now the dominant choice
- Applied to Q,K only (not V), before attention computation

**ALiBi (linear bias — MPT, BLOOM):**
- Adds position-dependent bias to attention scores
- No positional embedding added to input at all
...
**Causal mask (decoder):**
Token i can only attend to tokens 0..i. Implemented by adding -∞ to upper triangle of attention scores before softmax.
...
- d_model = 768, h = 12 heads, d_ff = 3072, N = 12 layers, vocab = 30522

**Per layer:**
- Q, K, V projections: 3 × (768 × 64) × 12 heads = 3 × 768 × 768 = ~1.77M
- Output projection: 768 × 768 = 590K
- FFN W₁: 768 × 3072 = 2.36M
- FFN W₂: 3072 × 768 = 2.36M
...
2048 tokens = 4M attention scores per head. 128K context = 16B scores. This is why long context is expensive and why FlashAttention exists (fused CUDA kernel, same math, 2-4× faster with HBM-aware tiling).

**Pre-norm ≠ Post-norm in training dynamics.**
Post-norm (original paper) requires careful LR warmup or the gradients through LayerNorm blow up early. Pre-norm (modern) trains more stably. If you see loss spikes early, check norm placement.

**Positional encoding mismatch at inference.**
Fine-tuned on 512 tokens, deployed on 2048 → absolute positional embeddings outside training range → degraded performance. RoPE handles this better (relative), still needs NTK-aware scaling for large extension.

**Attention head collapse.**
Many heads learn near-identical patterns. Common in undertrained models. Fix: entropy regularization on attention weights or prune redundant heads.

**d_k must be divisible by h.**
d_model=512, h=8 → d_k=64. If not divisible, projection matrices are misaligned. PyTorch's `nn.MultiheadAttention` handles this internally but check if implementing from scratch.

**Padding token attention leaks information.**
If you don't pass `key_padding_mask`, your model attends to [PAD] tokens. Pass the mask — especially critical in classification where [CLS] token must not aggregate padding noise.

**KV cache in inference.**
During autoregressive generation, recomputing K,V for all previous tokens at each step is O(N²). KV cache stores K,V for all past tokens → O(N) per new token. Essential for production inference. Cache size = 2 × N_layers × seq_len × d_model × batch_size × 2 bytes (fp16).
...
### Minimal Transformer Encoder Block (PyTorch)
```python
import torch
...
A: If Q, K ~ N(0,1) with dimension d_k, their dot product has variance d_k (sum of d_k independent unit-variance products). Softmax of large values → near one-hot distribution → gradients nearly zero for non-max positions. Dividing by √d_k keeps variance ≈ 1 regardless of d_k, keeping softmax in a numerically stable regime.

**Q: Encoder vs decoder — when do you pick each in production?**
A: For **understanding** (classification, NER, retrieval/embedding) → encoder-only (BERT variants) — bidirectional context gives richer representations. For **generation** → decoder-only (GPT/LLaMA) — causal training naturally learns to produce coherent sequences. For **seq2seq** (translation, summarization where input/output differ significantly) → encoder-decoder (T5/BART). In document understanding (OCR + NLP), LayoutLM/BERT variants dominate because you need joint understanding of text + spatial layout, not generation.

**Q: What's the actual complexity bottleneck in transformers at inference?**
A: Two bottlenecks: (1) **Attention is O(N²·d_model)** per layer in computation and memory — quadratic in sequence length. At N=4096, this is ≈67M attention scores per head. FlashAttention fuses the softmax+matmul into one CUDA kernel avoiding materializing the full NxN matrix in HBM. (2) **Memory bandwidth for loading weights** — at inference (batch=1), matmuls are memory-bound, not compute-bound. Large models are bottlenecked by how fast you can load weights from HBM → why quantization (INT8/INT4) helps inference latency.

**Q: Why does pre-norm train more stably than post-norm?**
A: In post-norm, the residual path goes through LayerNorm: LayerNorm(x + sublayer(x)). Early training when sublayer outputs are large → LayerNorm normalizes a large residual → large gradients through the norm → instability. In pre-norm, the residual path is clean: x + sublayer(LayerNorm(x)). The identity shortcut carries gradient directly without passing through LayerNorm → stable gradient flow from the start. You can train pre-norm with a higher LR and less warmup.

**Q: What's the difference between self-attention and cross-attention? Where is cross-attention used?**
A: In self-attention, Q, K, V all come from the same sequence (x·Wq, x·Wk, x·Wv — same x). In cross-attention, Q comes from one sequence (decoder), K and V come from another (encoder output). Cross-attention is in the decoder block of encoder-decoder models (T5, BART): the decoder queries the encoder's representations to condition generation on the source. Also used in: diffusion model UNets (queries from noisy image, keys/values from text conditioning), LLaVA-style vision-language models.

**Q: What happens to BERT-style models when input exceeds max_position_embeddings?**
A: Absolute positional embeddings are a learned table of size [max_seq_len, d_model]. Any position beyond that has no embedding → error or garbage output. Solutions: (1) Truncate input (crude, information loss). (2) Interpolate position embeddings (fine-tune at extended length). (3) Use RoPE-based models (LLaMA) which can be extended via NTK-aware scaling with less degradation. This is a fundamental limitation of BERT-style absolute position embeddings for long-document tasks.

**Q: Why do large decoder-only models (LLaMA-class) outperform encoder-decoder models (T5) on most benchmarks now?**
A: Scale + causal LM is a simpler optimization target. At massive scale (70B+), causal LM on trillions of tokens produces representations powerful enough for any downstream task via few-shot prompting or instruction tuning. Encoder-decoder has the advantage that the encoder sees full context — but this advantage shrinks as decoders get longer context windows and RLHF fine-tuning aligns outputs better. Practically: GPT-4/LLaMA-70B outperform T5-11B on most tasks, but T5/BART still win on structured seq2seq tasks (translation, constrained generation) at similar compute.

**Q: How does the KV cache work and when does it help/hurt?**
A: During autoregressive generation, at step t you compute K, V for all t tokens — but tokens 0..t-1 were already computed at prior steps. KV cache stores K, V from all past steps, so each new token only computes Q for the new token and reuses cached K, V. This reduces generation from O(N²) recomputation to O(N) total. Hurt: memory grows linearly with generated length — at 100K context, cache size = 2 × 32 layers × 100K × 128 d_head × 32 heads × 2 bytes ≈ 52GB for LLaMA-2-70B. Techniques: PagedAttention (vLLM), sliding window attention (Mistral) to bound cache size.
...
|-----------|----------|-----|
| Scaled dot-product attention | `../fundamentals/05_modern_components.md` | Full math derivation with toy example |
| LayerNorm in pre-norm pattern | `../fundamentals/03_training_stability.md` | LayerNorm vs BatchNorm vs RMSNorm detail |
| Residual connections | `../fundamentals/05_modern_components.md` | Gradient highway math |
| Positional encoding | `../fundamentals/05_modern_components.md` | Embeddings section — static vs contextual |
| Optimizer choice for transformers | `../fundamentals/02_training_loop.md` | AdamW is standard for transformers; why Adam's weight decay breaks |
| Document understanding (LayoutLM) | Your domain work | LayoutLM = BERT + 2D positional encoding for bounding boxes |
| Diffusion models | Future: `../architectures/05_diffusion.md` | Cross-attention for text conditioning |

---

## Key Takeaway

The transformer = **attention (route information between tokens) + FFN (transform each position) + residual + norm, stacked N times**.
...
For your domain (document understanding): BERT-class encoders are your backbone. Know how attention masks work, why [CLS] works for classification, and how to handle variable-length documents (truncation vs sliding window vs long-context models like Longformer/BigBird).

The three numbers to internalize: **O(N²) attention complexity**, **d_ff = 4×d_model**, **√d_k scaling**.
