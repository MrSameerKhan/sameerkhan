...
Same sentence throughout: **"cat sat on mat"**
Same 2D embeddings throughout:
- cat = [1.0, 0.5], sat = [0.2, 0.3], on = [0.1, 0.1], mat = [0.2, 0.4]
...
**Forward pass:**
1. Input → transformer → hidden states
2. Take [CLS] hidden state (BERT) or last token (GPT)
3. Project to task output with small head
4. Compute task loss
5. Backprop through head → through all transformer layers → update everything

### 1.2 Dry-Run: Sentiment Classification

Task: "cat sat on mat" → sentiment (positive=1, negative=0)
Label: positive (y=1)
...
**New prediction:**
```
z_new = 1.386×0.503 + 2.019×0.304
      = 0.697 + 0.614
      = 1.311

ŷ_new = σ(1.311) = 0.787
L_new = -log(0.787) = 0.240   (was 0.241) ✓
```

This gradient continues through the transformer layers too — every weight (Q, K, V, W_O, FFN, LayerNorm) gets updated.
...
Optimizer states (Adam): 7B × 8 bytes = 56 GB  ← two momentum terms
─────────────────────────────────────────────────
...
With adapters:
  x → Attention → Add+Norm → [Adapter] → FFN → Add+Norm → [Adapter] → output
```
...
For d=768 (BERT-base), r=64:
- W_down: 768×64 = 49,152 params
- W_up:   64×768 = 49,152 params
- Per adapter: 98,304 params

BERT-base has 12 layers × 2 adapters per layer = 24 adapters × 98,304 = ~2.4M trainable params vs 110M total. That's 2.2% of model size.
...
**Intuition:** Instead of modifying the model, you're adding a "context header" that steers the model's behavior.

For r=10 prefix tokens, d=768, L=12 layers:
- Params: 2 × 10 × 768 × 12 = 184,320 ≈ 0.17% of BERT

### 2.3 Prompt Tuning

Even simpler: just prepend learned embeddings to the INPUT (not every layer). Only the input embeddings are learnable.

For 20 learned tokens, d=768: 20 × 768 = 15,360 params — essentially zero.
...
- A is a r×d matrix (trainable)
- r << d (rank r is much smaller than dimension d)

**Why is B×A low-rank?** A d×d matrix has rank up to d. B×A has rank at most r. So you're saying: "The adaptation needed for this task lives in an r-dimensional subspace of the full d-dimensional space."
...
**Setup:**
- d=2 (our toy dimension)
- r=1 (rank-1 adaptation)
- Applying LoRA to W_Q (query weight matrix)
...
```
A = [[0.2, 0.1]]       shape: 1×2   (r×d)
B = [[0.0],            shape: 2×1   (d×r)
     [0.0]]
```

**Initialization rule:**
- A: initialized with small random values (Gaussian with σ = 1/r)
- B: initialized to ZERO

**Why B=0?** So that ΔW = B×A = 0 at the start. The model begins fine-tuning in exactly the pretrained state. If A were also zero, gradients wouldn't flow — A must be non-zero.
...
lora_output = [0.000] @ A = [0.000] @ [[0.2, 0.1]] = [0.000, 0.000]
...
lora_output = [0.020] @ [[0.2, 0.1]] = [0.004, 0.002]
...
The query vector changed by [0.004, 0.002] — a small, targeted adjustment. After many steps, this accumulates into a meaningful task-specific shift.
...
For r=8, α=16: scaling factor = 16/8 = 2.0
For r=8, α=8:  scaling factor = 1.0

**Why?** When you change r, the magnitude of B×A changes (more vectors → more capacity). α/r normalizes the contribution so that changing r doesn't require retuning the learning rate.
...
Original LoRA paper (Hu et al. 2021): apply to W_Q and W_V only.
Later work: applying to all attention weights (W_Q, W_K, W_V, W_O) and sometimes FFN weights gives better results.
...
Even with LoRA, the base model must be loaded in memory to compute forward/backward pass. For a 7B model:
```
...
For our weight W_Q = [[0.1, 0.2], [0.3, 0.4]]:
- All values are small (near 0) — NF4 represents these with higher precision than uniform quantization would
...
CLM pretraining teaches the model to predict tokens. But predicting the next token ≠ being helpful, harmless, or honest. A model trained only on internet text learns to:
- Continue rants and conspiracy theories
- Complete harmful instructions (more tokens follow harmful text online)
- Be verbose and evasive (that's what internet text looks like)
...
**For "cat sat on mat" (toy task):**
- Prompt: "continue the poem: cat"
- Target response: "sat on mat"
- CLM loss on: sat, on, mat
- L_sft = (L_sat + L_on + L_mat) / 3
...
**Data collection:**
For the same prompt, collect multiple model responses and ask humans to rank them:
```
Prompt:    "continue: cat sat on"
Response A: "mat"            → human ranks this #1 (on-topic, complete)
Response B: "the ground"     → human ranks this #2 (grammatically fine, less poetic)
Response C: "and then flew"  → human ranks this #3 (implausible, confusing)
```

**Reward model architecture:**
- Take the pretrained LM
- Replace the LM head with a scalar head: W_r (d×1)
- For each (prompt, response) pair, output one reward scalar r ∈ ℝ

**Training objective (Bradley-Terry preference model):**
For a preference pair (chosen response c, rejected response r):
```
...
Gradient: W_r gets updated to increase r_A and decrease r_C.

For preference pair (A preferred over B):
```
L_reward = -log σ(0.880 - 0.660) = -log σ(0.220) = -log(0.555) = 0.589
```

After training on many preference pairs, the reward model assigns higher scores to responses humans prefer.
...
```
KL(π_θ || π_sft) = Σ π_θ(y|x) × log(π_θ(y|x) / π_sft(y|x))
```

For our toy case, at position 2 ("on" → predicting next token):
```
π_sft  (SFT model):   P(mat)=0.400, P(cat)=0.300, P(sat)=0.200, P(on)=0.100
π_θ    (RL model):    P(mat)=0.600, P(cat)=0.200, P(sat)=0.150, P(on)=0.050
```
...
PPO requires:
1. The reward model (separate model to maintain)
2. Online sampling (generate responses during training — slow)
3. Complex RL training loop (hyperparameter-sensitive)
...
**DPO objective:**
```
L_DPO = -E[(c,r)] log σ(β × log(π_θ(c|x)/π_ref(c|x)) - β × log(π_θ(r|x)/π_ref(r|x)))
```
...
```
P_ref(cat sat on mat | prompt) = 0.15
P_ref(cat sat on ground | prompt) = 0.08
```
...
LoRA's claim: fine-tuning updates are intrinsically low-rank. Evidence:
- Train a full fine-tuned model, compute SVD of ΔW = W_ft - W_pretrained
- Look at the singular values — most are near zero

```
Singular values of ΔW (typical):
σ₁ = 0.843  ← most of the signal
σ₂ = 0.412
σ₃ = 0.071
σ₄ = 0.023
...
...
The first few singular vectors capture ~95% of the adaptation. LoRA with r=8 keeps the top 8 — enough for most tasks.
...
Before LoRA update:
```
q_on = x_on @ W_Q = [1.009, -0.316] @ [[0.1,0.2],[0.3,0.4]]
     = [1.009×0.1 + (-0.316)×0.3, 1.009×0.2 + (-0.316)×0.4]
     = [0.101 - 0.095, 0.202 - 0.126]
     = [0.006, 0.076]
```

After LoRA step (B_new from Section 3.4):
```
ΔW = [[0.010, 0.005], [-0.004, -0.002]]

x_on @ ΔW = [1.009, -0.316] @ [[0.010, 0.005],[-0.004,-0.002]]
           = [1.009×0.010 + (-0.316)×(-0.004), 1.009×0.005 + (-0.316)×(-0.002)]
           = [0.010+0.001, 0.005+0.001]
           = [0.011, 0.006]

q_on_new = q_on + [0.011, 0.006] = [0.017, 0.082]
```
...
### 10.1 Full Fine-Tuning (Classification Head)
...
# Backward pass
dz = y_hat - y                        # ∂L/∂z
dW_cls = x_cls[:, None] * dz         # x^T × dz, shape: (2,1)
db_cls = np.array([dz])
...
class LoRALayer:
    ...
    def forward(self, x):
        """x: (seq_len, d) or (d,)"""
        # Original weight path (frozen — no gradient flows here in practice)
        out_original = x @ self.W                   # (d,) @ (d,d) = (d,)
        
        # LoRA path (trainable)
        # x @ B @ A = low-rank update
        lora = x @ self.B @ self.A                  # (d,) @ (d,r) @ (r,d) = (d,)
    ...
    def backward(self, x, grad_out, lr=0.01):
        ...
        # Gradient w.r.t. B: ∂L/∂B = x^T × (grad_out_scaled × A^T)
        # More precisely: dL/dB = x.T @ (g * A^T considered via chain rule)
        xA = x @ self.B  # intermediate: (r,) [or scalar if r=1]
        dB = np.outer(x, g @ self.A.T)              # (d, r)
        dA = np.outer(xA, g)                         # (r, d)

        # Update
        self.B -= lr * dB
        self.A -= lr * dA
        
        # Gradient w.r.t. x (to pass upstream)
        # ∂L/∂x = g @ W.T (frozen) + g @ (B@A).T (LoRA)
        dx_original = grad_out @ self.W.T
        dx_lora     = g @ (self.B @ self.A).T
...
**Dry-run trace (numpy seed=42, d=2, r=1):**
```
Initial values (seed=42):
  W  = [[0.496, 0.715], [-0.383, 0.602]]   ← frozen, never changes
  B  = [[ 0.047], [-0.035]]                 ← d×r = 2×1
  A  = [[-0.063,  0.048]]                   ← r×d = 1×2
  B@A = [[0.047×-0.063, 0.047×0.048],       ← ΔW before training
         [-0.035×-0.063, -0.035×0.048]]
      = [[-0.00296,  0.00226],
         [ 0.00221, -0.00168]]   ← tiny, near zero (B init random, A init random)

Forward pass: x = [1.0, 1.5]
  W·x    = [0.496×1 + 0.715×1.5,  -0.383×1 + 0.602×1.5]
           = [1.569,  0.520]
  B@A·x  = [(-0.00296×1 + 0.00226×1.5),  (0.00221×1 + (-0.00168)×1.5)]
           = [0.00042,  -0.00031]
  output = [1.569 + 0.00042,  0.520 + (-0.00031)]
           = [1.569,  0.520]   ← essentially W·x at start (B@A ≈ 0)

Backward pass: grad_out = [-0.5, 0.2]
  g = grad_out × scale = [-0.5, 0.2] × 1.0 = [-0.5, 0.2]

  xA = x @ B  = [1.0, 1.5] @ [[0.047], [-0.035]]
             = 1.0×0.047 + 1.5×(-0.035) = 0.047 - 0.052 = -0.005

  dB = outer(x, g @ A.T)
     = outer([1.0, 1.5], [-0.5, 0.2] @ [[-0.063], [0.048]])
     = outer([1.0, 1.5], [-0.5×(-0.063) + 0.2×0.048])
     = outer([1.0, 1.5], [0.0315 + 0.0096])
     = outer([1.0, 1.5], [0.0411])
     = [[0.0411], [0.0617]]
  ...
  Update (lr=0.1):
    B_new = B - lr×dB = [[0.047], [-0.035]] - 0.1×[[0.0411], [0.0617]]
                      = [[0.047-0.00411], [-0.035-0.00617]]
                      = [[0.04289], [-0.04117]]

    A_new = A - lr×dA = [[-0.063, 0.048]] - 0.1×[[0.0025, -0.001]]
                      = [[-0.063-0.00025, 0.048+0.0001]]
                      = [[-0.06325, 0.04810]]

  ΔW after 1 step = B_new @ A_new:
    = [[0.04289], [-0.04117]] @ [[-0.06325, 0.04810]]
    = [[-0.00271,  0.00206],
       [ 0.00260, -0.00198]]

  W is still [[0.496, 0.715], [-0.383, 0.602]] ← UNCHANGED
  Effective W_total = W + ΔW = [[0.493, 0.717], [-0.380, 0.600]]
```
...
# Saves ~1.2MB instead of 440MB for full BERT
```
...
# Log probabilities of "sat on mat" (chosen) and "and then flew" (rejected)
...
### 10.6 Quick Reference: Hyperparameters

```python
# LoRA typical settings by model size

# < 1B (BERT-base, DistilBERT)
lora_small = {"r": 8,  "alpha": 16, "target": ["query", "value"], "lr": 3e-4}

# 1-7B (LLaMA-2-7B, Mistral-7B)
lora_medium = {"r": 16, "alpha": 32, "target": ["q_proj","v_proj","k_proj","o_proj"], "lr": 2e-4}

# > 7B (LLaMA-2-70B)
lora_large = {"r": 64, "alpha": 128, "target": ["q_proj","v_proj","k_proj","o_proj","gate_proj","up_proj","down_proj"], "lr": 1e-4}

# QLoRA on top of any of the above
qlora_extra = {
    "load_in_4bit": True,
...
**1. Catastrophic forgetting in full fine-tuning**
Fine-tuning on a small dataset can destroy the model's general capabilities. A BERT fine-tuned on 1000 sentiment examples may lose performance on NER, QA, etc. Solutions: lower learning rate, fewer epochs, early stopping, or use PEFT.

**2. LoRA doesn't help with compute at inference if not merged**
Unmerged LoRA adds two extra matrix multiplications per adapted layer. For high-throughput serving, always merge: W_merged = W₀ + (α/r) × B×A.

**3. LoRA rank r=0 doesn't mean "no adaptation"**
Setting r=0 would mean no LoRA matrices at all — it doesn't silently fall back to full fine-tuning. If you want no adaptation, explicitly freeze all parameters.

**4. A is initialized non-zero but B is zero — the asymmetry matters**
If both A and B were zero, ∂L/∂A = ∂L/∂z × B = 0 and ∂L/∂B = ∂L/∂z × A = 0 — both matrices would stay at zero forever. Initializing A to small random values breaks this symmetry.

**5. The reference model in DPO must stay frozen**
The DPO objective computes log(π_θ / π_ref). If π_ref is the same model being updated (not frozen), the ratio stays near 1 and the loss gives no signal. Always freeze a separate copy as π_ref.

**6. RLHF reward hacking is real and subtle**
The reward model is trained on human ratings, but human raters have limited attention spans. A long, confident-sounding response often gets higher ratings than a short, accurate one — even if the long one is less useful. This is why RLHF models sometimes produce "padded" responses.

**7. SFT data quality >> quantity**
InstructGPT used 13K high-quality SFT examples. Adding more low-quality examples hurt. For RLHF pipelines, the SFT stage is the foundation — bad SFT data propagates into reward model training and RL optimization.

**8. KL divergence direction matters for RLHF**
The KL penalty uses KL(π_θ || π_ref) (forward KL), not KL(π_ref || π_θ) (reverse KL). Forward KL penalizes probability mass that π_θ puts on responses π_ref assigns low probability to — it keeps the model from generating very unusual (potentially harmful) outputs.

**9. LoRA on attention only vs attention + FFN**
LoRA on Q,V only was the original paper's recommendation. In practice, including K,O and FFN weights (all linear layers) often improves performance at the cost of more parameters. For a specific task, ablate: start with Q,V, add others if validation loss plateaus.
...
A: The pre-training task (MLM or CLM) trains models to be maximally general — the weight matrices span a broad subspace. Fine-tuning on a narrow task only needs to shift representations along a few relevant directions. Aghajanyan et al. (2021) measured the "intrinsic dimensionality" of fine-tuning and found that for most tasks, fewer than 200 directions suffice — even for 100M+ parameter models.

**Q: Does merging LoRA after fine-tuning affect model quality?**  
A: No, merging is mathematically exact: W₀ + (α/r)×B×A is equivalent to using the two separate matrices. The only difference is practical: merged weights load as a single matrix (standard model format), unmerged requires the PEFT library to intercept forward passes.

**Q: Why does RLHF use PPO instead of a simpler optimization method?**  
A: The RL objective involves generating sequences — the model must sample responses, score them with the reward model, and use those scores to update. This requires an RL algorithm that works with sampled (non-differentiable) actions. PPO handles this via the importance sampling ratio. DPO avoids this by working with pre-collected preferences (no online generation needed).

**Q: What is the difference between instruction tuning (SFT) and RLHF?**  
A: Instruction tuning trains on (instruction, good response) pairs with supervised CLM loss. The model learns to imitate good responses. RLHF goes further: the model learns to prefer better responses over worse ones via human comparison data, and explicitly optimizes for a reward signal. Instruction tuning is a prerequisite for RLHF (you need a capable model before you can rank its outputs).

**Q: Can you apply LoRA to embedding layers?**  
A: Yes, but it's unusual. For token embeddings (vocab_size × d), LoRA would be: E_new = E₀ + B×A where B is vocab_size×r and A is r×d. This is uncommon because: (1) embeddings are already looked up, not matrix-multiplied; (2) most tasks don't need to shift token meanings. Positional embeddings are even less common LoRA targets since they're fixed (sinusoidal) in BERT/GPT-style models.

---

## 13. Connections

**To pretraining objectives (fundamentals/04_pretraining_objectives.md):**
- SFT continues CLM training with high-quality demonstrations — same objective, different data distribution
- BERT-style models fine-tune best with adapter/LoRA on attention weights — because MLM representations are already classification-ready
- GPT-style models go through SFT → RLHF/DPO for alignment — building on the CLM representation

**To BERT end-to-end (models/05_bert_end_to_end.md):**
- [CLS] hidden state x_cls = [1.386, 2.019] used directly as classification input in Section 1.2
- The classification head math here matches the "Fine-tuning" section there exactly
- Fine-tuning only the [CLS] projection is the simplest PEFT approach for BERT

**To GPT end-to-end (models/06_gpt_end_to_end.md):**
- CLM loss values (2.604, 3.101, 2.120) are the SFT starting point
- KV cache makes RLHF inference tractable — essential for online PPO generation
- Sampling strategies (top-p, temperature) are how RLHF generates response candidates

**To transformer architecture (fundamentals/02_transformer_architecture.md):**
- LoRA targets Q,K,V,O — all attention projections defined in that file
- The FFN W₁, W₂ matrices are secondary LoRA targets
- LayerNorm γ,β are excluded from LoRA (already tiny; full fine-tuning of these is common)

**To tokenization (fundamentals/03_tokenization.md):**
- Full fine-tuning can optionally update token embeddings — sometimes beneficial for domain-specific vocabulary
- RLHF reward hacking is partly a tokenization artifact: padding tokens inflate sequence length
- DPO log probabilities are summed over tokens — tokenization affects response probability magnitude
